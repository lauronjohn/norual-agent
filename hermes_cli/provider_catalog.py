...
The provider list shown by ``norual model`` (CLI/TUI) and the desktop Settings
...
the same universe ``norual model`` renders (``CANONICAL_PROVIDERS``), joining:
...
``CANONICAL_PROVIDERS`` universe, i.e. exactly what ``norual model`` shows.
"""
...
class ProviderDescriptor:
    ...
    order: int                     # CANONICAL_PROVIDERS index — mirrors `norual model`
...
def provider_catalog() -> list[ProviderDescriptor]:
    """Return one descriptor per provider in the ``norual model`` universe.
    ...
    # Norual overlays carry auth_type for providers that have no registry/profile
    ...
    for order, entry in enumerate(CANONICAL_PROVIDERS):
        ...
        # Norual overlay (e.g. moa → "virtual"), then api_key.
        auth_type = (
