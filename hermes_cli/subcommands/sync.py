"""``norual sync`` subcommand parser — Skill Sync.
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    norual sync status                 show gate/opt-in/head state
    norual sync pull                   pull and materialize opted-in skills
    norual sync push                   push opted-in skills
    norual sync now                    reconcile: pull then push
    norual sync enable <skill>         opt a skill into sync
    norual sync disable <skill>        opt a skill out of sync
    norual sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    norual sync propose <skill>        share a skill with your organisation
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  norual sync status            what is synced, and from where\n"
            "  norual sync enable my-skill   include a skill in your sync\n"
            "  norual sync now               pull, then push\n"
            "  norual sync propose my-skill  share a skill with your team\n"
        ),
