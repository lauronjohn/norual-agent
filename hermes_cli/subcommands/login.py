"""``norual login`` subcommand parser.
...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    ``norual login`` was removed in favor of ``norual auth`` / ``norual model``
    ...
    old scripts/aliases invoking ``norual login [--flags]`` still receive the
    ...
    - The subparser is registered WITHOUT a ``help=`` kwarg so the row is
      omitted from ``norual --help`` (argparse only lists subcommands that
    ...
    - ``--provider`` accepts ANY value (no ``choices=``) so that, e.g.,
      ``norual login --provider anthropic`` reaches the deprecation handler and
      gets pointed at ``norual model`` instead of crashing in argparse with
      ``invalid choice: 'anthropic'`` before the handler can run.
    """
    login_parser = subparsers.add_parser(
        "login",
        description=(
            "Deprecated. Use `norual auth` to manage credentials, "
            "`norual model` to select a provider, or `norual setup` for full setup."
        ),
    )
    ...
    login_parser.add_argument(
        "--provider",
        default=None,
        help="(deprecated) Provider name; ignored — see `norual model`",
    )
    login_parser.add_argument(
