"""``norual dashboard`` / ``norual serve`` subcommand parsers.
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `norual dashboard` / `norual serve` cmdlines and SIGTERM them directly —
    # the same path `norual update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Norual web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Norual web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Norual Agent web dashboard for managing config, API keys, and sessions",
    )
    _add_server_runtime_args(dashboard_parser)
    dashboard_parser.add_argument(
        "--no-open", action="store_true", help="Don't open browser automatically"
    )
    # Backward-compat shim: older Norual desktop app shells (<= 0.15.x) spawn the
    # backend as `norual dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Norual couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the Norual Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Norual backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the Norual backend server — the JSON-RPC/WebSocket gateway the "
    ...
    # `norual dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.hermes/.env.
    # Nested subparser so bare `norual dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "how to engage the login gate. Requires being logged in (norual setup)."
        ),
    )
