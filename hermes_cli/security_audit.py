"""On-demand supply-chain audit for Norual Agent installs.

Scans three surfaces a Norual user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Norual venv (every PyPI dist via ``importlib.metadata``).
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `norual security audit`."""
