"""Cross-process mutual exclusion for in-flight Norual updates.

Three different surfaces can start an update of the same install tree:

* ``norual update`` from a terminal,
...
once — so a dashboard-spawned ``norual update`` and an installer-driven
...
then spawns ``norual update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("Norual is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ancestry check those users get exit 2 ("Norual is still running") on every
  GUI update forever, with no Norual process actually running.
"""
...
# its own pid before spawning `norual update` as a child stage. The parent
...
# to show "Norual is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Norual home (never the context-local profile override):
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``norual update`` as a (grand)child, so a
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Norual update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `norual update` as a stage): we run under ITS claim
