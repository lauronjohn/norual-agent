...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or norual -c.",
    ...
    "Ctrl+Z suspends Norual to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "norual -c resumes your most recent CLI session. norual -c \"project name\" resumes by title.",
    "norual -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "norual -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "norual chat -t web,terminal enables only specific toolsets for a focused session.",
    "norual chat -s github-pr-workflow preloads a skill at launch.",
    "norual chat -q \"query\" runs a single non-interactive query and exits.",
    "norual chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "norual chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "norual --yolo bypasses all dangerous command approval prompts for the entire session.",
    "norual chat --source telegram tags the session for filtering in norual sessions list.",
    "norual -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "norual doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "norual dump outputs a compact setup summary — great for bug reports.",
    "norual config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "norual config edit opens config.yaml in your default editor.",
    "norual config check scans for missing or stale configuration options.",
    "norual sessions browse opens an interactive session picker with search.",
    "norual sessions stats shows session counts by platform and database size.",
    "norual sessions prune --older-than 30 cleans up old sessions.",
    "norual skills search react --source skills-sh searches the skills.sh public directory.",
    "norual skills check scans installed hub skills for upstream updates.",
    "norual skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "norual skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "norual mcp add github --command npx adds MCP servers from the command line.",
    "norual mcp serve runs Norual itself as an MCP server for other agents.",
    "norual auth add lets you add multiple API keys for credential pool rotation.",
    "norual completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "norual logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "norual backup creates a zip backup of your entire Norual home directory.",
    "norual profile create coder creates an isolated profile that becomes its own command.",
    "norual profile create work --clone copies your current config and keys to a new profile.",
    "norual update syncs new bundled skills to ALL profiles automatically.",
    "norual gateway install sets up Norual as a system service (systemd/launchd).",
    "norual memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "norual webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: norual tools disables unused tools, norual skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "norual models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as norual -w).",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Norual tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'norual profile create coder' creates the 'coder' command.",
    "norual profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Norual prints a resume command with session ID and stats.",
    "norual sessions export backup.jsonl exports all sessions for backup or analysis.",
    "norual -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "norual skills install official/security/1password installs optional skills from the repo.",
    ...
    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: norual cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Norual runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "norual gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Norual via a local macOS server.",
    ...
    "Norual loads project context from .hermes.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "norual mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "norual mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "norual mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — norual tools can toggle them per platform.",
    ...
    "norual chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "norual plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "norual acp runs Norual as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "norual auth add nous or norual auth add openai-codex sets up OAuth-based providers.",
    ...
    "norual status --deep runs deeper diagnostic checks across all components.",
    ...
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Norual your own.",
    ...
    "Before a gateway session reset, Norual auto-flushes important facts to memory in the background.",
    ...
    "Profiles scope Norual state via HERMES_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'norual profile create ls' would be rejected.",
    "norual profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "norual claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via norual tools.",
    ...
    '/goal <text> sets a standing Ralph-loop objective — Norual auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Norual config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Norual is working.',
    ...
    # --- Curator ---
    'norual curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "norual curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'norual curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'norual auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    'norual dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'norual dashboard embeds the full Norual TUI in your browser via xterm.js and a WebSocket PTY.',
    ...
    'norual -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'norual chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'norual chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'norual chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "norual chat --source tool tags programmatic chats so they don't clutter norual sessions list.",
    'norual dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'norual sessions rename <ID> "new title" renames any past session; norual sessions delete <ID> removes one.',
    'norual import restores a full Norual backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'norual fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'norual pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'norual setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'norual status --deep runs the full health sweep across every component; plain norual status is the quick view.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Norual block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from norual sessions list by default — set --source explicitly to see them.',
...
# any project or none — Norual is not a coding-only agent.
# ---------------------------------------------------------------------------
