"""norual webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    norual webhook subscribe <name> [options]
    norual webhook list
    norual webhook remove <name>
    norual webhook test <name> [--payload '{"key": "value"}']
...
def _setup_hint() -> str:
  ...
  Then start the gateway: norual gateway run
"""
...
def webhook_command(args):
    """Entry point for 'norual webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: norual webhook {subscribe|list|remove|test}")
        print("Run 'norual webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (norual gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: norual webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from norual webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (norual gateway run)")
