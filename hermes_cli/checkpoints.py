"""`norual checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.hermes/checkpoints/``.  Actions:

    norual checkpoints               # same as `status`
    norual checkpoints status        # total size, project count, breakdown
    norual checkpoints list          # per-project checkpoint counts + workdir
    norual checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    norual checkpoints clear [-f]    # nuke the entire base (asks first)
    norual checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    norual checkpoints
    norual checkpoints prune --retention-days 3 --max-size-mb 200
    norual checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: norual checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``norual checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `norual checkpoints` → status
