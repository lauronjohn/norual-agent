...
entries via ``norual mcp catalog`` or the interactive ``norual mcp picker``,
and install them with ``norual mcp install <name>`` (or by toggling in the
...
- Manifests pin transport details (commands, args, refs). Pins follow the
  ...
  auto-updated; users explicitly re-run ``norual mcp install <name>`` to
...
def _catalog_root() -> Path:
    """Return the optional-mcps/ directory shipped with this Norual install."""
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if mv != _MANIFEST_VERSION:
        raise CatalogError(
            ...
            f"(this Norual understands version {_MANIFEST_VERSION})"
        )
...
def list_catalog() -> List[CatalogEntry]:
    ...
    UIs can tell the user their Norual is out of date.
    """
    root = _catalog_root()
    if not root.exists():
        return []
...
def catalog_diagnostics() -> List[tuple]:
    ...
    is one of:
      - ``future_manifest`` — manifest_version is newer than this Norual
        understands. Update Norual to install this entry.
...
def _apply_tool_selection(
    ...
    *,
    ...
    Probe-fail path:
      ...
      - Either way, point the user at ``norual mcp configure <name>``.
    """
    print()
    ...
    if entry.tools.default_excluded and prior_selection is None:
        if prior_exclude is not None:
            ...
            print(color(
                ...
                "config.yaml or run "
                f"`norual mcp configure {entry.name}` to change.",
                Colors.GREEN,
            ))
            return
    ...
    if probed is None:
        ...
        if prior_selection is not None:
            ...
            print(color(
                ...
                f"Run `norual mcp configure {entry.name}` after the server "
            ...
            print(color(
                ...
                f"Run `norual mcp configure {entry.name}` after first "
                "connect to prune.",
                Colors.YELLOW,
            ))
        return
    ...
    if not chosen_indices:
        ...
        print(color(
            f"  No tools selected. Run `norual mcp configure {entry.name}` "
            "to change.",
            Colors.YELLOW,
        ))
        return
    ...
    if len(chosen_indices) == len(probed):
        ...
        # the user can re-run `norual mcp configure <name>` and unselect a
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    if entry.auth.type == "api_key":
        ...
        if entry.auth.provider:
            ...
            # the existing `norual auth <provider>` flow. Surface guidance
            ...
            print(color(
                ...
                f"`norual auth {entry.auth.provider}` if you have not "
    ...
    print(color(
        ...
        f"Start a new Norual session to load its tools.",
        Colors.GREEN,
    ))
