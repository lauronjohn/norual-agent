"""
Gateway subcommand for norual CLI.

Handles: norual gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# when hermes_constants is cached from a pre-update version during `norual update`.
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "norual CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
# — restart may fail", and `norual update` can deadlock behind it.  The loop
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``norual gateway status`` from falsely counting the ``hermes`` CLI that
...
def _scan_gateway_pids(
    ...
    # scan (e.g. ``norual gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    try:
        if is_windows():
            ...
            # wedges the caller forever. ``norual update`` hung exactly there
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``norual update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Norual profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Norual profiles via PID files."""
    _exclude = set(exclude_pids or set())
...
def _prepare_profile_gateway_update_restart(profile: str, pid: int) -> str | None:
    """Choose who relaunches a profile gateway after ``norual update``.

    A gateway started with ``--external-supervisor`` must exit back to that
    manager. Starting Norual's detached watcher as well would escape the
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # when the user closes the terminal, leaving ``norual update`` users
    # with no running gateway until they re-invoke ``norual gateway``
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}norual gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}norual gateway restart{scope_flag}"
    )
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    if _launchd_unsupported_marker_exists():
        ...
        print("  Stop it with: norual gateway stop")
    else:
        print(
            "⚠ Gateway process is running for this profile, but the service is not active"
        )
        print(f"  PID(s): {_format_gateway_pids(snapshot.gateway_pids, limit=None)}")
        print("  This is usually a manual foreground/tmux/nohup run, so `norual gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``norual gateway status`` output so users with
...
def _reap_unsupervised_gateway_orphans(extra_exclude: set | None = None) -> bool:
    ...
    if is_windows():
        try:
            ...
            # dormant on every standard `norual gateway install` deployment.
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Norual Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``norual gateway run`` must remain interruptible from
...
SERVICE_DESCRIPTION = "Norual Agent Gateway - Messaging Platform Integration"
...
def _profile_arg(hermes_home: str | None = None, default_root: str | Path | None = None) -> str:
    ...
    Args:
        ...
        default_root: Optional Norual root to compare against. Used when
...
# Legacy service names from older Norual installs that predate the
...
def _find_legacy_hermes_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Norual gateway units.

    Detects unit files installed by older Norual versions that used a
...
def has_legacy_hermes_units() -> bool:
    """Return True when any legacy Norual gateway unit files exist."""
    return bool(_find_legacy_hermes_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Norual gateway unit files if any are installed.
    ...
    if not legacy:
        return
    print_warning("Legacy Norual gateway unit(s) detected from an older install:")
    ...
    print_info("    norual gateway migrate-legacy")
...
def remove_legacy_hermes_units(
    ...
    legacy = _find_legacy_hermes_units()
    if not legacy:
        print("No legacy Norual gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Norual gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: norual gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo norual gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    norual gateway uninstall")
    print_info("    sudo norual gateway uninstall --system")
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            print_warning(
                ...
                "root shell, or install a user service instead: norual gateway install"
            )
            return scope, False
...
def ensure_gateway_service(context: str = "setup") -> bool:
    ...
    The zero-decision service path used by ``norual setup`` (end of wizard)
    and ``norual import``: if this host supports a service manager and no
    ...
    ``norual gateway install --system`` explicitly (that path prompts and
    ...
    if is_container():
        ...
        print_info("   norual gateway run          # Run as container main process")
        print_info("")
    ...
    if not (supports_systemd or is_macos() or is_windows()):
        ...
        print_info("  Run the gateway in the foreground with: norual gateway")
        return False
    ...
    try:
        ...
        print_info("  You can retry manually: norual gateway install")
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Norual often sets ``HOME`` to a profile-scoped directory, but
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /opt/norual                 -> /opt/norual  (kept as-is)
...
def _hermes_home_for_target_user(target_home_dir: str) -> str:
    ...
    root's home.  This translates it to the target user's equivalent path:
      /root/.norual                    → /home/alice/.hermes
      /root/.hermes/profiles/coder     → /home/alice/.hermes/profiles/coder
      /opt/custom-norual               → /opt/custom-norual  (kept as-is)
    """
    ...
    # Default ~/.norual → remap to target user's default
    if current_hermes == current_default:
        return str(target_default)

    # Profile or subdir of ~/.norual → preserve the relative structure
    try:
...
def _stable_service_working_dir() -> str:
    ...
    dir, or a clone that ``norual update`` later relocates/removes — the path
...
def _append_node_dir_for_service(
    ...
    The Norual-managed Node under ``$HERMES_HOME/node`` goes first when it
    ...
    *hermes_root* is the Norual home the unit will run against. System units
    ...
    # target Norual home provides managed Node, consulting the invoker's PATH
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if not system:
        ...
        # user's Norual home is known — probing here would stat the calling
...
def systemd_unit_is_current(system: bool = False) -> bool:
    ...
    #
    # Under ``sudo norual gateway … --system``, HERMES_HOME is often stripped
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``norual gateway restart``
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Norual install"
    )
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo norual gateway uninstall --system")
    print_info("         norual gateway install")
    print_info("         norual gateway start")
...
def systemd_install(
    ...
    # ``sudo norual gateway install --system --force`` would bake /root/.hermes
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}norual gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}norual gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}norual gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `norual gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `norual gateway status` or logs for final state."
            )
            return
        _wait_for_systemd_service_restart(system=system, previous_pid=pid)
        return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}norual gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}norual gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}norual gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}norual gateway start{scope_flag}"
        )
...
# process invocation (each ``norual gateway start/stop/status`` call).
...
#      background process (the `nohup norual gateway run` workaround). See #23387.
...
def _timestamped_stderr_gateway_command(
    ...
    *,
    ...
    ``norual update`` sees the flag on the live grandchild argv and hands
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup norual gateway run --replace`
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    print(
        f"  Try manually: nohup norual gateway run --replace "
...
def generate_launchd_plist() -> str:
    ...
    # without this block every plist rewrite (e.g. `norual gateway start`)
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current Norual install"
    )
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  norual gateway status             # Check status")
    from hermes_constants import display_hermes_home as _dhh
...
def launchd_stop():
    ...
    # `norual gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_restart():
    ...
    try:
        ...
        if pid is not None and probe_gateway_loop_liveness(pid) == GATEWAY_LOOP_WEDGED:
            ...
            # full drain budget only stalls the restart (and `norual update`
...
def launchd_status(deep: bool = False):
    ...
    # Norual PID tracking — may be a detached fallback process spawned when
    ...
    # launchd PID and the Norual PID file).
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Norual install")
    else:
        print("⚠ Service definition is stale relative to the current Norual install")
        print("  Run: norual gateway start")
    ...
    if service_listed:
        if launchd_pid is not None:
            ...
            if fallback_pid:
                ...
                print("  Cron jobs will fire. Stop with: norual gateway stop")
            else:
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``norual gateway run`` is always a misconfiguration, so we
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``norual gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``norual gateway run`` can burn memory/CPU just to fail with "already
    ...
    print("  Use 'norual gateway restart' to replace it,")
    print("  or 'norual gateway stop' first.")
    print("  Or use 'norual gateway run --replace' to auto-replace.")
    sys.exit(1)
...
def _guard_official_docker_root_gateway() -> None:
    ...
    print_error(
        "Refusing to run the Norual gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'hermes' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/hermes/docker/entrypoint.sh before the Norual command."
    )
    print(
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `norual gateway run` still
    ...
    # /restart) rather than through `norual gateway restart` which already
    ...
    # `norual gateway start/restart` — leaving the gateway vulnerable to
    ...
    def _hard_exit_after_gateway_teardown(code: int) -> None:
        # ``norual gateway run`` enters through this CLI wrapper, not through
...
_PLATFORMS = [
    ...
    # plugins/platforms/matrix/adapter.py::register(). #41112.
    {
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Channel ID where Norual delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Norual connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: norual pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Norual will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``norual setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run norual under
        WSL.
    """
            ...
            if value:
                ...
                if is_email:
                    ...
                    access_choices = [
                        ...
                        "Use DM pairing (unknown users request access, you approve with 'norual pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: norual pairing approve <platform> <code>"
                    )
                elif is_email:
                    print_success("  Unknown email senders will be ignored.")
                else:
                    print_info(
                        "  Skipped — configure later with 'norual gateway setup'"
                    )
            continue
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Norual will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. Norual will store the returned account_id/token in ~/.hermes/.env."
    )
    print_info(
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `norual gateway setup`.")
        return
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `norual pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `norual pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "NorualAgent"')
...
def gateway_setup():
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: norual gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: norual gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: norual gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo norual gateway install --system"
                        )
                    print_info("  Or run in foreground:  norual gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: norual gateway run")
                print_info(
                    "  For persistence:   tmux new -s norual 'norual gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: norual gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup norual gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'norual gateway setup' when ready.")
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``norual gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``norual gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    #
    # Prefer `sleep infinity` (matches the static main-norual service's
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: norual gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: norual gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s norual 'norual gateway run'"
                )
                print()
            ...
            print()
            print(
                "  norual gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s norual 'norual gateway run'         # persistent via tmux"
            )
            print(
                "  nohup norual gateway run > ~/.hermes/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  norual profile create <name>     # creates the s6 service slot")
                print("  norual -p <name> gateway start   # bring it up via s6")
                print("  norual status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: norual gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: norual gateway run")
            sys.exit(1)
        ...
        if _is_supervised_gateway_process():
            print_error(
                ...
                "Use `norual gateway uninstall` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: norual gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  norual profile delete <name>     # tears down the s6 service slot")
                print("  norual -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `norual -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: norual gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        # The supervised probe also PASSES a plain foreground `norual gateway run`
        ...
        if _is_supervised_gateway_process():
            print_error(
                ...
                "Use `norual gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if _is_supervised_gateway_process():
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `norual gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: norual gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  norual gateway install")
                else:
                    print("To install as a service:")
                    print("  norual gateway install")
                    print("  sudo norual gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  norual gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup norual gateway run > ~/.hermes/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s norual 'norual gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  norual gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  norual gateway install  # Install as user service")
                    print(
                        "  sudo norual gateway install --system  # Install as boot-time system service"
                    )
        ...
        # Stop, disable, and remove legacy Norual gateway unit files from
