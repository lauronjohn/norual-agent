"""
Interactive setup wizard for Norual Agent.
...
# display_hermes_home imported lazily at call sites (stale-module safety during norual update)
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Norual Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Norual using environment variables or config commands:")
    print_info("  norual config set model.provider custom")
    print_info("  norual config set model.base_url http://localhost:8080/v1")
    print_info("  norual config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'norual setup' in an interactive terminal to use the full wizard.")
    print()
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'norual setup')")
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if not _provider_ready:
        print()
        print_warning("No inference provider is configured — Norual cannot chat yet.")
        print_info("  Finish this one step with either of:")
        print_info("    norual model            (pick any provider/model)")
        print_info("    norual setup --portal   (Nous Portal OAuth, no API key)")
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'norual setup' to configure"))
    ...
    # Video generation — opt-in via `norual tools` → Video Generation.
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'norual setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'norual setup tts'"))
    else:
    ...
    if _stt_feature is not None and _stt_feature.managed_by_nous:
        ...
        if fw_ok:
            ...
            tool_status.append(
                ("Speech-to-Text (Local Whisper — not installed)", False, "run 'norual tools' → Speech-to-Text")
            )
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'norual setup terminal'"))
    ...
    # Spotify (OAuth via norual auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'norual setup tools' to configure them,"
        )
    ...
    print()
    print(f"   {color('norual setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('norual setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('norual setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('norual setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('norual setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('norual config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('norual config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('norual config set <key> <value>', Colors.GREEN)}")
    ...
    print(f"   {color('norual gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('norual doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `norual tools` and `norual setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``norual model``)
    ...
    provider added to ``norual model`` is automatically available here.
    ...
    # Delegate to the shared norual model flow — handles provider picker,
    ...
    try:
        ...
        print_info("You can try again later with: norual model")
    ...
    # on demand via `norual auth add`, `norual setup` vision, and
    # `norual setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``norual model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "neutts":
        ...
        # via the Norual auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via norual setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'norual setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Norual runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `norual setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `norual setup terminal`.
        ...
        if prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
            ...
            print_info(
                "Run `norual egress setup` then `norual egress start` to mint "
                "tokens and launch the proxy."
            )
        else:
            print_info(
                "Skipping egress firewall. You can enable it later with `norual egress setup`."
            )
        ...
        try:
            ...
            # which() misses the uv Norual installed. Bootstrapping one is
...
def _apply_default_agent_settings(config: dict):
    ...
    print_info("  Run `norual setup agent` later to customize.")
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Norual delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Norual to iMessage via BlueBubbles — a free, open-source")
...
def _setup_webhooks():
    ...
    print()
    print_info("   Open config in your editor:  norual config edit")
    print_info("   Open config in your editor:  norual config edit")
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Norual from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'norual setup gateway' later to configure.")
    else:
        for idx in selected:
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     norual config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
    ...
    # via `norual import` or `norual setup gateway`). Gating this on
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `norual setup tools` and `norual tools` use the same flow:
...
def setup_telemetry(config: dict):
    ...
    print_info("Packages stay under this Norual profile and are not uploaded.")
...
# Config values may have different semantics between OpenClaw and Norual.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Norual to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Norual at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Norual at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Norual at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Norual at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Norual equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Norual config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Norual.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Norual's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    print_info("Norual can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: norual claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: norual claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Norual configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Norual config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Norual (use norual claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``norual setup --portal`` and ``norual portal``. This is the
    ...
    Norual session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``norual setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``norual model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``norual portal`` always offers a model picker,
    ...
    print(color("│     ⚕ Norual Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # setup calls, so `norual portal` == quick setup's Nous step.
    try:
        ...
        print_info("  You can retry later with `norual portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `norual portal`: %s", exc)
        print()
    ...
    print_info("  Run `norual portal info` to inspect routing.")
...
def run_setup_action_with_navigation(
    ...
    *,
    ...
    Shared commands such as ``norual model`` use the same provider/model
...
def _run_setup_wizard_impl(args):
    ...
    Supports full, quick, and section-specific setup:
      norual setup           — full or quick (auto-detected)
      norual setup model     — just model/provider
      norual setup tts       — just text-to-speech
      norual setup terminal  — just terminal backend
      norual setup gateway   — just messaging platforms
      norual setup tools     — just tool configuration
      norual setup telemetry — just local shared metrics
      norual setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Norual Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Norual Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Norual Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Norual configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'norual setup model|terminal|")
        ...
        setup_mode = prompt_choice(
            "How would you like to set up Norual?",
            [
    ...
    print()
    print_info("You can edit these files directly or use 'norual config edit'")
    ...
    # Tune later with `norual setup agent`.
    if not is_existing:
...
def _run_first_time_quick_setup(config: dict, hermes_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``norual setup <section>``
    or switch providers with ``norual model``.
    """
    from hermes_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'norual setup gateway'",
        ],
        0,
    )
    ...
    if gateway_choice == 0:
        ...
        # (e.g. via `norual import` from another machine).
        from hermes_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
    ...
    print()
    print_info("  Configure all settings:    norual setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  norual setup gateway")
    print()
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    configuring, and troubleshooting Norual) is always seeded — without
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``norual tools`` (which
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``norual setup agent`` /
    ``norual config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, hermes_home, is_existing: bool):
    ...
    print_info("can help you drive and configure Norual itself.")
    print()
    ...
    if path == 0:
        ...
        # `norual update` runs don't re-inject them. Essential skills (the
        ...
        print_info("  Enable tools:        norual tools")
        print_info("  Seed skills:         norual skills opt-in --sync")
        print_info("  Add MCP servers:     norual mcp add")
        print_info("  Enable plugins:      norual plugins")
        print_info("  Tune agent settings: norual setup agent")
        print()
        _print_setup_summary(config, hermes_home)
        return
...
def _blank_slate_walkthrough(config: dict, hermes_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`norual update` runs from re-injecting them. Opt back in any")
            print_info("time with `norual skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `norual tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `norual plugins list` / `norual plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `norual plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `norual mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `norual mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   norual tools")
    print_info("  Seed skills:         norual skills opt-in --sync")
    print_info("  Add MCP servers:     norual mcp add")
    print_info("  Tune agent settings: norual setup agent")
    print()
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'norual setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Norual to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'norual setup gateway'.")
