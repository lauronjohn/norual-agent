"""``norual dashboard register`` — register a self-hosted dashboard OAuth client.
...
This command:
  1. Resolves a fresh Nous Portal access token from the existing login
     ...
     "run `norual setup`" hint when the user isn't logged in.
...
def _register_self_hosted_client(
    *,
    ...
    makes re-running ``norual dashboard register`` idempotent. The portal
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                ...
                "Try `norual auth add nous` to re-authenticate."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def _print_post_register_hint(
    *,
    ...
    print(
        ...
        "  `norual dashboard` (localhost) leaves the gate off and serves locally\n"
    ...
    if custom_redirect_uri:
        ...
        print("    norual dashboard --host 0.0.0.0")
...
def cmd_dashboard_register(args) -> None:
    ...
    if is_managed():
        print(
            "✗ `norual dashboard register` is not available in a managed/hosted "
            "install.\n"
    ...
    try:
        ...
        if getattr(exc, "relogin_required", False):
            ...
            print("  Run `norual setup` (or `norual auth add nous`) first, then retry.")
        else:
