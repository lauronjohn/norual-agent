"""
MCP Server Management CLI — ``norual mcp`` subcommand.

Implements ``norual mcp add/remove/list/test/configure`` for interactive
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``norual mcp login`` to distinguish a genuine authentication
    ...
    """
    try:
        from tools.mcp_oauth import NorualTokenStorage
        return NorualTokenStorage(name).has_cached_tokens()
...
def cmd_mcp_add(args):
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  norual mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  norual mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  norual mcp add myserver --preset mypreset')
        return
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info("Fix the issue, then: norual mcp test " + name)
        return
...
def cmd_mcp_remove(args):
    ...
    # earlier `norual mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  norual mcp add <name> --url <endpoint>')
        _info('  norual mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def _reauth_oauth_server(name: str, server_config: dict) -> bool:
    ...
    landed before reporting success. Shared by ``norual mcp login`` and
    ``norual mcp reauth`` so both behave identically for a single server.
    """
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `norual mcp remove` + `norual mcp add` to reconfigure auth.")
        return False
    ...
    #
    # force_interactive_oauth: `norual mcp login` is *explicitly* user-
    # initiated even when stdin isn't a TTY (Norual desktop / agent-
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `norual mcp login " + name + "`.")
            return False
        if tools:
...
def cmd_mcp_reauth(args):
    ...
    ``norual mcp reauth <name>`` re-auths a single server (same as ``login``).
    ``norual mcp reauth --all`` discovers every ``auth: oauth`` server in
    ...
    if not name:
        ...
        _info("Usage: norual mcp reauth <name>   |   norual mcp reauth --all")
        return
    if name not in servers:
        _error(f"Server '{name}' not found in config.")
        if servers:
...
# ─── norual mcp configure ────────────────────────────────────────────────────

def cmd_mcp_configure(args):
    """Reconfigure which tools are enabled for an existing MCP server."""
    import sys as _sys
    if not _sys.stdin.isatty():
        print("Error: 'norual mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``norual mcp`` subcommands."""
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `norual plugin`.
        from hermes_cli.mcp_picker import run_picker
        run_picker()
        print(color("  Commands:", Colors.CYAN))
        _info("norual mcp                                    Open the catalog picker (default)")
        _info("norual mcp catalog                            List Nous-approved MCPs")
        _info("norual mcp install <name>                     Install a catalog MCP")
        _info("norual mcp serve                              Run as MCP server")
        _info("norual mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("norual mcp add <name> --command <cmd>         Add a stdio server")
        _info("norual mcp add <name> --preset <preset>       Add from a known preset")
        _info("norual mcp remove <name>                      Remove a server")
        _info("norual mcp list                               List configured servers")
        _info("norual mcp test <name>                        Test connection")
        _info("norual mcp configure <name>                   Toggle tools")
        _info("norual mcp login <name>                       Re-authenticate OAuth")
        _info("norual mcp reauth <name> | --all              Re-auth one or all OAuth servers")
        print()
