"""MCP picker — interactive `norual mcp picker` (also the default `norual mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``norual mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `norual plugin` picker UX: arrow keys to navigate, ENTER on a row
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Norual session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `norual mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: norual mcp install <name>    Picker: norual mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Norual is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Norual — run `norual update` "
...
def show_catalog() -> None:
    """`norual mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`norual mcp picker` (and default `norual mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`norual mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `norual mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
