...
manual BotFather token copy-paste. Norual talks only to the Nous onboarding
...
DEFAULT_MANAGER_BOT = "NorualSetupBot"

DEFAULT_BOT_NAME = "Norual Agent"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Norual Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Norual Telegram onboarding service.")
