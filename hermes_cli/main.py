#!/usr/bin/env python3
"""
Norual CLI - Main entry point.

Usage:
    norual                     # Interactive chat (default)
    norual chat                # Interactive chat
    norual gateway             # Run gateway in foreground
    norual gateway start       # Start gateway as service
    norual gateway stop        # Stop gateway service
    norual gateway status      # Show gateway status
    norual gateway install     # Install gateway service
    norual gateway uninstall   # Uninstall gateway service
    norual setup               # Interactive setup wizard
    norual logout              # Clear stored authentication
    norual status              # Show status of all components
    norual cron                # Manage cron jobs
    norual cron list           # List cron jobs
    norual cron status         # Check if cron scheduler is running
    norual doctor              # Check configuration and dependencies
    norual honcho setup                    # Configure Honcho AI memory integration
    norual honcho status                   # Show Honcho config and connection status
    norual honcho sessions                 # List directory → session name mappings
    norual honcho map <name>               # Map current directory to a session name
    norual honcho peer                     # Show peer names and dialectic settings
    norual honcho peer --user NAME         # Set user peer name
    norual honcho peer --ai NAME           # Set AI peer name
    norual honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    norual honcho mode                     # Show current memory mode
    norual honcho mode [hybrid|honcho|local]  # Set memory mode
    norual honcho tokens                   # Show token budget settings
    norual honcho tokens --context N       # Set session.context() token cap
    norual honcho tokens --dialectic N     # Set dialectic result char cap
    norual honcho identity                 # Show AI peer identity representation
    norual honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    norual honcho migrate                  # Step-by-step migration guide: OpenClaw native → Norual + Honcho
    norual --version           Show version and update status
    norual update              Update to latest version
    norual uninstall           Uninstall Norual Agent
    norual acp                 Run as an ACP server for editor integration
    norual sessions browse     Interactive session picker with search

    norual claw migrate --dry-run  # Preview migration without changes
"""
...
# When the user upgrades code via ``git pull`` (or ``norual update``
...
# this guard, norual crashes on import and the user can't run
# ``norual update`` to recover.  Missing the bootstrap means UTF-8 stdio
...
# a prior ``norual update`` left a recovery marker and a core package's import
...
def _set_process_title() -> None:
    ...
    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``norual tools`` or
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    cron jobs, pipes run ``norual … chat -q`` with stdio on a pipe. This
...
def _suppress_mouse_residue_early() -> None:
    ...
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`norual --tui … >log`, CI capture):
...
def _try_ultrafast_version() -> bool:
    """Handle ``norual --version`` before config/logging imports."""
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (norual tools, norual setup, norual model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'norual {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any norual module import.
#
...
def _apply_profile_override() -> None:
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `norual mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to Norual' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo norual -p <name>` against the invoking user's home.
    ...
    # after the subcommand (`norual chat -p coder`), so keep scanning broadly.
    ...
    # /opt/data/profiles/coder).  If HERMES_HOME points to the norual root
    # instead (e.g. systemd hardcodes HERMES_HOME=/root/.hermes), we must
    # still read active_profile — the user may have switched profiles via
    # `norual profile use` and the gateway should honour that choice.
    # See issue #22502.
    ...
    # 2. If no flag, check active_profile in the norual root.
    #
    ...
    # reserved ``gateway-default`` slot runs bare ``norual gateway run`` to mean
    ...
    if profile_name is not None:
        try:
            ...
            # A bug in profiles.py must NEVER prevent norual from starting
            print(
...
# Norual root's ``bin``, next to the managed uv) by install.ps1. That dir
# lives OUTSIDE the git checkout precisely because an earlier layout staged
# the copies at ``<checkout>\bin``, where ``norual update``'s autostash
...
# profile override on purpose — no norual module may be imported before
...
def _has_any_provider_configured() -> bool:
    ...
    # Determine whether Norual itself has been explicitly configured (model
    ...
    # Only count these if Norual has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Norual to use their tokens.
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    ``norual -c`` from repo A continues repo A's last session rather than the
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo norual {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo norual {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print(
                    "Use 'norual sessions list' to see available sessions, or "
...
def _print_tui_exit_summary(
    ...
    print(f"  norual --tui --resume {target}")
    if title:
        print(f'  norual --tui -c "{title}"')
    print()
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``norual update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    print(
        "Error: the TUI workspace is missing from this Norual checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `norual update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Norual checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `norual --tui`\n"
        "If the checkout is still inconsistent, run `norual update --force`.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    def _node_bin(bin: str) -> str:
        ...
        # and exit on an install whose only Node is the one Norual installed,
    ...
    if (
        ...
        if result.returncode != 0:
            ...
            # here before doing any work; repair once (upgrade a Norual-managed
...
def _launch_tui(
    ...
    # TUI child is a norual process: propagate the profile-home contract via
    ...
    # in the user's shell would otherwise make a plain `norual --tui` try to
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `norual update` so
...
def _pin_kanban_board_env() -> None:
    ...
    (``norual kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``norual kanban boards switch`` from another session can flip the file
...
def _sync_bundled_skills_quietly() -> None:
    ...
    interaction with Norual — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Norual still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    workers, cron jobs, and pipelines run ``norual … chat -q`` with stdout
...
def cmd_chat(args):
    ...
    if isinstance(_resume_raw, str) and _resume_raw.strip().lower() == "latest":
        ...
        if _last_id:
            ...
            print("Use 'norual sessions list' to see available sessions.")
            sys.exit(1)
    ...
    # Code / Codex CLI) and resume the newly created Norual session.
    ...
    if isinstance(_resume_foreign, str) and _resume_foreign.strip().lower() in (
        ...
        print(f"  (later: norual --resume {_imported_id})")
    ...
    try:
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'norual doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Norual isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  norual setup")
        print()
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'norual setup' at any time to configure.")
        sys.exit(1)
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Norual?")
        print()
    ...
    # has no creds.json.  Every subsequent `norual gateway` then paid a 30s
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: norual gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `norual gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  norual gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Norual Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  norual gateway")
        ...
        print()
        print("  Or install as a service: norual gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'norual whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``norual whatsapp`` (the Baileys bridge wizard) — the
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``norual model``) and the setup wizard
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'norual model' for "
                "available providers, or run 'norual doctor' to diagnose config "
                "issues."
            )
    ...
    # Honor ``model_catalog.excluded_providers`` so the CLI ``norual model``
...
#
# Norual uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `norual model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `norual model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Norual only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    up new ones through the normal ``norual model`` flow, then route aux tasks
    to them here.
    """
    from hermes_cli.config import load_config
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Norual URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
# fast paths like `norual --version` and slash-command dispatch that never
...
# every `hermes` invocation, including fast paths like `norual --version`
...
def _prompt_api_key(
    ...
    """Shared API-key entry point for ``norual setup`` / ``norual model``.
    ...
    if choice.startswith("c") and not pool_backed:
        ...
        print(
            f"  API key cleared.  Re-run `norual setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print(
                f"    Norual will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate Norual CLI with a provider."""
...
def cmd_sync(args):
        ...
        if status.get("org_available"):
            ...
            if modified:
                print(
                    ...
                    f"{', '.join(modified)}\n"
                    f"  Share them back with `norual sync propose <skill>`. "
    ...
    try:
        if sub == "pull":
            ...
            result = ssc.push_skills(identity=identity, message="norual sync push")
        elif sub == "now":
            pull_res = ssc.pull_skills(identity=identity)
            push_res = ssc.push_skills(identity=identity, message="norual sync now")
...
def cmd_slack(args):
    ...
    Dispatches ``norual slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: norual slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `norual slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1
...
def cmd_security(args):
    """Dispatch `norual security <subcmd>`."""
...
def cmd_approvals(args):
    """Dispatch `norual approvals <subcmd>`."""
...
def cmd_backup(args):
    """Back up Norual home directory to a zip file."""
...
def cmd_import(args):
    """Restore a Norual backup from a zip file."""
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    # `norual --version` pre-import fast path (the `version` subcommand
...
def cmd_uninstall(args):
    """Uninstall Norual Agent (or just the Chat GUI with --gui)."""
...
def _sweep_stale_bytecode_if_checkout_changed() -> None:
    ...
    ``norual update``, a manual ``git pull``, a ZIP update, a file-sync
    ...
    Update-time clears alone can never close this class: ``norual update``
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    mtime comparison. ``git checkout`` / ``git pull`` / ``norual update``
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the norual venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — norual venv python3, no nix-shell overhead
    ...
    # a norual venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``norual update`` to stash the
    lockfile — repeatedly.
    ...
    # of a Norual-managed install, or a freshly provisioned managed npm when
...
def _do_build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``norual web``).
    ...
    # as `norual update`'s _update_node_dependencies() (ui-tui + web +
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (norual web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``norual update`` can unconditionally call ``norual desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``norual desktop`` (interactive launch) skips the build when the
...
def _renderer_bundle_dir(desktop_dir: Path, *, source_mode: bool) -> Optional[Path]:
    ...
    # macOS: …/Norual.app/Contents/MacOS/Norual → …/Contents/Resources
    resources = (
...
def _renderer_bundle_torn(dist_dir: Path) -> bool:
    ...
    the intact SOURCE tree, ``norual desktop`` skips the rebuild that would fix
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``norual update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Norual.app/Contents/MacOS/Norual"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Norual.exe",
            release_dir / "win-ia32-unpacked" / "Norual.exe",
            release_dir / "win-arm64-unpacked" / "Norual.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "hermes",
            release_dir / "linux-unpacked" / "Norual",
            release_dir / "linux-arm64-unpacked" / "hermes",
            release_dir / "linux-arm64-unpacked" / "Norual",
        ]
    ...
    if sys.platform == "win32" and len(existing) > 1:
        ...
        # Picking purely by mtime can then hand a wrong-architecture Norual.exe
...
# `norual update` → `norual desktop --build-only` → relaunch) rebuilds
# Norual.exe on the end user's machine and used to verify only that the file
...
# wrong-architecture Norual.exe is caught BEFORE the updater replaces the
...
def _windows_native_machine_from_iswow64() -> Optional[str]:
    ...
    process arch) and rejected a correctly-built ARM64 ``Norual.exe``.
...
def _ensure_desktop_exe_launchable(
    ...
    print(f"✗ The built Norual.exe failed its integrity check: {error}")
    ...
    if restored is not None:
        print("  ↩ Update aborted — restored the previous working Norual.exe from backup.")
        print("    Your existing version was kept and still works. Run `norual desktop`")
        print("    (or the in-app update) again to retry with a fresh Electron download.")
        return restored, True

    print("  ✗ No usable backup was found to restore.")
    print("    Run `norual desktop --force-build` to rebuild, or re-run the Norual")
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Norual'`` desktop build failure: a corrupt zip in the
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Norual.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Norual.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Norual.exe: Access is
    ...
    an unrelated "Norual" process is never touched. Best-effort: never raises.
...
def _desktop_macos_local_codesign(
    ...
    #    contains the outer Norual.app component, so an absolute-parts check
...
def _desktop_macos_relaunchable_fixup(
    ...
    *,
    ...
    "Norual is damaged and can't be opened" — and macOS TCC forgets every
    ...
    if exe is None:
        return True
    # exe = .../Norual.app/Contents/MacOS/Norual  ->  app bundle = .../Norual.app
    app = exe.parents[2]
...
def _desktop_macos_setup_tcc_identity(identity: str = "Norual Local Signing") -> bool:
    """Create/import a self-signed code-signing cert and configure Norual to use it.

    One-shot setup for ``norual desktop --setup-tcc-identity``. Creates a
    ...
    # Point Norual at the identity (config.yaml, not .env — it's not a secret).
    try:
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``norual desktop --build-only`` → electron-builder ``--dir``).
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Norual Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Norual Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def _detect_linux_password_store() -> str | None:
    ...
    fails under `norual desktop` because the launcher environment doesn't look
...
def _register_linux_desktop_entry() -> None:
    """Install the XDG desktop entry for Norual Desktop (Linux only, best-effort).
    ...
    shell. ``norual uninstall --gui`` removes it.
    """
    try:
...
def cmd_gui(args: argparse.Namespace):
    ...
    # `HERMES_DESKTOP_DISABLE_GPU=... norual desktop` and
    # `ELECTRON_OZONE_PLATFORM_HINT=... norual desktop` keep working.
    ...
    # `HERMES_DESKTOP_PASSWORD_STORE=... norual desktop` keeps working.
    ...
    if getattr(args, "setup_tcc_identity", False):
        identity = getattr(args, "identity", None) or "Norual Local Signing"
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  norual gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            # Put the Norual-managed Node on PATH so npm's child scripts (which
            ...
            # norual update) loses shell PATH customizations. Wrapping the
            # NixOS build env keeps its PYTHON hint while restoring managed Node
            # ahead of a bare PATH (same idiom as the `norual update` path).
            ...
            if not source_mode:
                ...
                # holds Norual.exe locked on Windows, so the pack can't replace
            ...
            if (
                ...
                if restored:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Norual.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Norual.exe, close any")
                    print("  running Norual desktop window and retry.")
                print("  If the log shows Electron download retries, rebuild via a mirror:")
                print("    ELECTRON_MIRROR=<mirror-base-url> norual desktop --force-build")
                sys.exit(build_result.returncode or 1)
            packaged_executable = _desktop_packaged_executable(desktop_dir)
            if not source_mode:
                # Locally-built apps are ad-hoc signed; make them relaunchable after
                # an in-place self-update (otherwise macOS reports "Norual is
                ...
                # success on a Norual.exe Windows cannot load (truncated PE from
    ...
    # Linux: register the app in the desktop launcher, so Norual shows up
    ...
    if source_mode:
        print("→ Launching Norual Desktop from source build...")
    ...
    print(f"→ Launching packaged Norual Desktop: {' '.join(launch_command)}")
...
def _restart_managed_dashboard_service(
    ...
    # Probe the user manager first: Norual installs Linux services in the
...
def _dashboard_cmdline_for_pid(pid: int) -> list[str] | None:
    ...
    is reconstructed, but norual launch commands don't embed exotic args).
...
def _respawn_dashboard_processes(commands: list[list[str]]) -> list[list[str]]:
    """Best-effort respawn of manually-started dashboards after ``norual update``.
...
# =========================================================================
# Fork detection and upstream management for `norual update`
# =========================================================================
...
def _recover_from_interrupted_install() -> None:
    """Finish update work left half-done by a prior ``norual update``.
    ...
    protocol stream (``norual acp`` speaks JSON-RPC on stdout) must never get
...
def _recover_core_update_marker_locked() -> None:
    ...
    """
    print(
        "⚠ A previous `norual update` was interrupted mid-install — "
    ...
    try:
        ...
        if self_locked:
            print(
                "  Norual is still running from the launcher that needs "
                "replacing. Close other Norual windows, restart from a "
...
def _windows_shim_in_process_chain() -> Path | None:
    ...
    ``norual ...`` command holds its own shim, and an editable install run
...
def _reexec_dependency_sync_off_windows_shim() -> bool:
    ...
    The child re-runs ``norual update``, so the whole remaining flow — the
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``norual update`` is still progressing even if pip/uv itself is silent.
    """
...
def _quarantine_running_hermes_exe(
    ...
    when ``norual update`` runs, ``hermes.exe`` IS the live process, and uv
    ...
    the next norual invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Norual Desktop backend child process
    ...
    2. If all retries fail, print a clear warning naming the most likely
       culprit (running Norual Desktop / gateway / REPL).
    ...
    for shim in _hermes_exe_shims(scripts_dir):
        ...
        print(
            "    Close Norual Desktop, exit other `hermes` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `norual update`."
        )
...
def _cleanup_pending_shim_renames(scripts_dir: Path) -> int:
    """Drop reboot renames older Norual versions queued for our shims.

    Norual used to fall back to ``MoveFileExW(MOVEFILE_DELAY_UNTIL_REBOOT)``
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    ...
    Called early on every norual invocation. Two cases the old unconditional
    ...
    2. **Concurrency.** A fresh quarantine file may belong to an update in
       ...
       ``norual update`` does exactly this). Leave anything inside the grace
...
def _repair_venv_via_import_probes(
    ...
    *,
    ...
    print("  ⚠ Venv repair incomplete. Run manually, then `norual update`:")
    print(
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    # hours later inside ``norual update``'s desktop-rebuild or skill-sync
...
def _verify_console_scripts_installed(
    ...
    *,
    ...
    ``hermes.exe`` shim is locked during ``norual update``, or when uv/distlib
    ...
    try:
        ...
        print(
            "  ⚠ Entry point repair failed; try `norual update --force` after "
            "closing other norual processes."
        )
        return
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``norual update``, not the venv we just wrote
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `norual update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `norual update --force` after closing other norual processes."
        )
        return
...
class _UpdateOutputStream:
    """Stream wrapper used during ``norual update`` to survive terminal loss.
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``norual update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``norual update`` in an SSH session or a terminal
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``norual update --gateway``) the update is already
...
def cmd_update(args):
    """Update Norual Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Norual Agent")
        return
    ...
    if getattr(args, "plan", False):
        ...
        # Norual runtime across profiles, its supervisor, and its running
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `norual update --branch=<x>` would actually pull.
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``norual -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `norual profile` — show current profile status
        ...
        if current is not None:
            ...
            if p.alias_path:
                ...
                print(f"Alias:          {alias_display} → norual -p {p.name}")
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  norual profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     norual -p {name} chat")
                else:
        ...
        if dist_name:
            ...
            print(f"  (run `norual profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → norual -p {name}  ({wrapper})")
        print()
        ...
        try:
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  norual -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      norual -p {plan.manifest.name} chat")
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `norual profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"norual -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        if data.get("hermes_requires"):
            print(f"Requires:     Norual {data['hermes_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    if mf.hermes_requires:
        print(f"  Requires: Norual {mf.hermes_requires}")
...
def _report_dashboard_status() -> int:
    ...
    if not live:
        print("No norual dashboard processes running.")
        return 0

    print(f"{len(live)} norual dashboard process(es) running:")
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    provider on the spot — or point them at ``norual dashboard register`` for
    OAuth.
    ...
    print("    [2] OAuth via Nous Portal (run `norual dashboard register`)")
    print("    [3] Cancel")
    print()
...
def _is_electron_packaged_web_dist(path: str) -> bool:
    ...
    ``.../app.asar.unpacked/dist``. A standalone ``norual dashboard`` that
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No norual dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `norual update`.
    ...
    if _token_file and not _headless_backend:
        raise SystemExit("--ssh-session-token-file is only valid with norual serve")
    ...
    # those vars then runs `norual dashboard` would otherwise:
    ...
    if (
        ...
        # HERMES_HOME falls back to $HOME/.norual = /opt/data/.norual — an
        ...
        # returns the root for both layouts: ~/.norual for a standard install
    ...
    # the same logs directory as every other Norual surface.
    try:
        from hermes_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
...
def cmd_logs(args):
    """View and filter Norual log files."""
...
def cmd_console(args):
    """Open the safe Norual command console."""
...
def _first_positional_argv() -> str | None:
    ...
    ``norual -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _resolve_deferred_platform_cli_command(command_name: str | None) -> None:
    ...
    registers a top-level ``norual <name>`` command (e.g. Photon ->
    ...
    import it, so the CLI registration never happens and ``norual photon``
...
def _try_fast_chat_launch() -> bool:
    ...
    ``hermes`` / ``norual -w -s foo --yolo`` / ``norual chat`` don't need the
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `norual --tui` is the hot path on phones. The full parser setup imports
...
def cmd_acp(args):
    """Launch Norual Agent as an ACP server."""
    try:
...
def _cmd_skills_trust(args):
    """``norual skills trust [path]`` / ``norual skills untrust [path]``.
...
def _advertise_agent_env() -> None:
    ...
    ``HERMES_AGENT`` is the Norual-specific marker. setdefault: never
    clobber an outer harness (e.g. Norual running inside another agent's
    terminal).
    """
...
def main():
    """Main entry point for norual CLI."""
    ...
    # ``norual update`` runs on Windows. Silent no-op on non-Windows or when
    ...
    # If the checkout changed since the last launch (norual update, manual
    ...
    # Self-heal a venv left half-built by an interrupted ``norual update``
    ...
    # ``norual skills install update``) merely defers recovery one launch;
    # under-matching (missing ``norual -p work update``) would race a recovery
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `norual model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    worktree_parser = subparsers.add_parser(
        ...
        description=(
            "Attended reclaim for the .worktrees/ directory norual -w sessions "
    ...
    # `norual proxy` (defined elsewhere in this file) is a separate INBOUND
    ...
    def _dispatch_egress(args):  # noqa: ANN001
        ...
        # from the inbound OAuth ``norual proxy`` subparser (dest='proxy_command').
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `norual whatsapp` which sets up "
    ...
    # =========================================================================
    # peer command — bot-to-bot DMs across machines (peer Norual gateways)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        ...
        "repo norual uses to snapshot working directories before "
    ...
    # subcommand — ``norual --help``, ``norual logs``,
    ...
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and Norual shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app."
        ),
    )
    try:
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Linux.\n\n"
            "Use `norual computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `norual tools` runs when you first\n"
            "enable the Computer Use toolset, and is a stable target\n"
            "for re-running the install if it didn't fire (e.g. when\n"
            "toggling the toolset on a returning-user setup).\n\n"
            "Use `norual computer-use doctor` to run cua-driver's\n"
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the Norual app. `status` reports the driver's grant\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                # to repair an install that `norual computer-use install` will
                ...
                if not contract.get("ready"):
                    ...
                    if override:
                        print(
                            ...
                            "the override and run: norual computer-use install --upgrade"
                        )
                    else:
                        print("    Run: norual computer-use install")
                    return 1
                try:
                    ...
                    if st and st.get("update_available"):
                        ...
                        print("    Run: norual computer-use install --upgrade")
                    elif st:
                        print("  ✓ Up to date.")
                    else:
                        # Older driver (no check-update verb) or offline.
                        print("  Refresh to latest: norual computer-use install --upgrade")
                except Exception:
                    print("  Refresh to latest: norual computer-use install --upgrade")
                return 0
        ...
        if action == "permissions":
            ...
            if perms_action == "status":
                ...
                if not st["platform_supported"]:
                    ...
                    sys.exit(1)
                if not st["installed"]:
                    print("cua-driver: not installed. Run: norual computer-use install")
                    sys.exit(1)
                ...
                if st["can_grant"]:  # macOS TCC permissions
                    ...
                    if not st["ready"]:
                        print("  Grant: norual computer-use permissions grant")
    ...
    sessions_export.add_argument(
        ...
        help=(
            ...
            "md/qmd: output directory (default: <norual home>/session-exports)"
        ),
    )
    sessions_export.add_argument(
        "--format",
    ...
    sessions_import = sessions_subparsers.add_parser(
        "import",
        help="Import a Claude Code or Codex CLI session into Norual",
        description=(
            "Pull a conversation started in Claude Code (~/.claude/projects) "
            "or Codex CLI (~/.codex/sessions) into the Norual session store "
            "so it can be resumed with 'norual --resume <id>'. The foreign "
    ...
    # NOTE: the `norual version` subcommand was removed — `norual --version`
    ...
    # for one release. The Norual-Setup.exe success screen tells users to
    # run `norual desktop` from a terminal, so the canonical name needs
    ...
    # e.g. ``norual -c Pokemon Agent Dev`` → ``norual -c 'Pokemon Agent Dev'``
    ...
    # fails (e.g. 'norual -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (norual hooks list, cron
    ...
    # ``norual egress start`` refusing when credential_source=bitwarden
