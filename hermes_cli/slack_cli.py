"""``norual slack ...`` CLI subcommands.

Today only ``norual slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ norual slack manifest > slack-manifest.json
    # or:
    $ norual slack manifest --write
...
def _build_full_manifest(
    ...
    it stays in sync with the rest of Norual. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Norual deployment — users can tweak them in the Slack UI after
    pasting.

    By default, this keeps Norual on Slack's older Assistant messaging
    ...
    if messaging_experience == "assistant":
        features["assistant_view"] = {
            "assistant_description": "Chat with Norual in threads and DMs.",
        }
        ...
        features["agent_view"] = {
            "agent_description": "Chat with Norual in Slack Messages.",
        }
        bot_scopes.append("assistant:write")
    ...
    display_information = {
        "name": bot_name[:35],
        "description": (bot_description or "Your Norual agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``hermes_cli/main.py``):
      ...
      --name NAME     Override the bot display name (default: "Norual")
    ...
    """
    name = getattr(args, "name", None) or "Norual"
    description = getattr(args, "description", None) or "Your Norual agent on Slack"
    ...
    if getattr(args, "slashes_only", False) and (
        ...
        print(
            "norual slack manifest: long description options cannot be used "
            "with --slashes-only",
            file=sys.stderr,
        )
        return 2
    if long_description_file is not None:
        ...
        try:
            ...
            print(
                f"norual slack manifest: cannot read long description from "
                f"{source_arg}: {exc}",
                file=sys.stderr,
            )
            return 2
    if (
        ...
        print(
            "norual slack manifest: long description must be at least "
    ...
    if (
        ...
        print(
            "norual slack manifest: long description must be at most "
    ...
    if write_target is not None:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Norual app\n"
