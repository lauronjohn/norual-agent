...
try:
    ...
    # running `norual dashboard` needs fastapi+uvicorn; lazy install keeps
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``norual gateway run`` — but the
    desktop app spawns a ``norual dashboard`` backend, not a gateway, so a cron
...
def _eager_reconcile_own_session_db() -> None:
    ...
    bringing a store left behind by `norual update` current before the
...
async def _lifespan(app: "FastAPI"):
    ...
    # so a store left behind by `norual update` kept 500ing every
    ...
    # model picker) can detect when `norual update` replaced the code
...
app = FastAPI(title="Norual Agent", version=__version__, lifespan=_lifespan)
...
_SESSION_HEADER_NAME = "X-Norual-Session-Token"
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Norual-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
def _dashboard_public_hosts() -> frozenset[str]:
    ...
    ``public_url`` is already Norual' canonical browser-facing URL behind a
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "proxy.enabled": {
        "type": "boolean",
        "description": (
            "Docker-only egress credential firewall. Requires `norual egress setup` "
            "and `norual egress start`; Modal/SSH/Daytona are not wired yet."
        ),
        "category": "security",
    },
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Norual (no terminal prompt), "
    ...
    "updates.refresh_cua_driver": {
        "type": "boolean",
        "description": (
            "Refresh an already-installed cua-driver during norual update. "
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real Norual provider slug — fine.
    ...
    1. Vendor-name → Norual-provider mapping: when the provider string is
       not a known Norual provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    # not be treated as a stray vendor prefix -- it isn't a known Norual
...
def _is_sensitive_filename(name: str) -> bool:
    ...
    canonical Norual credential-store basenames (see
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``norual update``.
    ...
    a container — e.g. the hermes-webui image sharing the Norual source tree),
    the dashboard's ``norual update`` button is the correct update path and
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's Norual root is actually /opt/data
...
async def upload_chat_image(payload: ChatImageUpload, profile: Optional[str] = None):
    ...
    The dashboard /chat page runs Norual inside an xterm.js PTY. Browser
...
# install, minted on first read and persisted under the ROOT Norual home
...
def _read_or_create_install_id() -> Optional[str]:
    """Read (or mint + persist) the install id under the root Norual home.
...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        # and which providers are registered so ``norual status`` and the
        ...
        # ``gateway_mode`` are low-sensitivity PRODUCT surface — Norual Cloud
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the norual home root).
    """
...
def _spawn_hermes_action(
    ...
    *,
    ...
    """Spawn ``norual <subcommand>`` detached and record the Popen handle.
    ...
    # _HERMES_GATEWAY=1. Inheriting it makes a spawned `norual gateway restart`
...
def _durable_completed_update_action_id(lines: List[str]) -> Optional[str]:
    ...
    for index, line in enumerate(lines):
        if line.startswith("=== norual update started "):
            last_start = index
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``norual gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``norual gateway restart`` children race each other on the
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``norual gateway restart`` in the background."""
    try:
...
async def update_hermes():
    """Kick off ``norual update`` in the background."""
    if _dashboard_local_update_managed_externally():
        message = (
            "Norual updates are managed outside this dashboard in "
    ...
    try:
        ...
        _log.exception("Failed to spawn norual update")
...
async def check_hermes_update(force: bool = False):
    """Report whether a Norual update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/hermes/update`` actually runs ``norual update``.

    Returns:
        install_method: 'apt' | 'git' | 'docker' | 'nix' | 'nixos' | 'unknown'
        current_version: installed Norual version string
    ...
    if install_method == "apt":
        payload["message"] = (
            "Norual is managed by Termux APT; run `pkg upgrade hermes-agent`."
        )
        return payload
...
async def get_update_receipt():
    ...
    if not receipt:
        raise HTTPException(
            status_code=404,
            detail="No update receipt found (no `norual update` run recorded).",
        )
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Norual supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def _install_memory_provider_pip_dependencies(dependencies: List[str]) -> List[Dict[str, Any]]:
    ...
    # under /opt/norual is sealed read-only, and installs must be redirected
...
def _dashboard_code_skew_guard() -> Optional[str]:
    ...
    boot.  When ``norual update`` (or a manual ``git pull``) replaces the
    ...
    return (
        ...
        f"(systemctl --user restart hermes-dashboard, or norual dashboard --port <port>)"
    )
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `norual model` does so GUI onboarding lands on a
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # endpoint, mirroring the ``norual model`` custom flow
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``norual model`` universe). This is what lets the desktop Keys tab
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock — a `norual model` provider — would be
        ...
        # Vertex — a `norual model` provider — would be invisible in the desktop
...
def _get_env_vars_sync(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict, *, custom: bool = False) -> dict:
        ...
        return {
            ...
            # CLI `norual model` picker uses (not desktop-only prefix guesses).
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Norual from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Norual to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Norual from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Norual to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Norual in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Norual through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Norual via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to Norual through an IMAP/SMTP mailbox.",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Norual to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Norual inside Feishu / Lark.",
    ...
    "google_chat": {
        "name": "Google Chat",
        "description": "Connect Norual to Google Chat via Cloud Pub/Sub.",
    ...
    "bluebubbles": {
        ...
        "description": "Use Norual through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Norual to a QQ Bot from the QQ Open Platform.",
    ...
    "teams": {
        "description": "Connect Norual to Microsoft Teams chats via the Bot Framework.",
    ...
    "irc": {
        "description": "Relay messages between an IRC channel (or DMs) and Norual.",
        "docs_url": "https://hermes-agent.nousresearch.com/docs/user-guide/messaging/irc",
    },
    "line": {
        "description": "Use Norual from LINE via the LINE Messaging API webhook.",
        "docs_url": "https://hermes-agent.nousresearch.com/docs/user-guide/messaging/line",
    },
    "ntfy": {
        "description": "Chat with Norual over ntfy push topics (ntfy.sh or self-hosted).",
        "docs_url": "https://hermes-agent.nousresearch.com/docs/user-guide/messaging/ntfy",
    },
    "photon": {
        "description": "Use Norual through iMessage via Photon's managed Spectrum platform.",
    ...
    "simplex": {
        "description": "Talk to Norual over SimpleX Chat via a local simplex-chat daemon.",
        "docs_url": "https://hermes-agent.nousresearch.com/docs/user-guide/messaging/simplex",
    },
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Norual to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose Norual as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://hermes-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
    ...
    "whatsapp_cloud": {
        ...
        "description": "Use Norual via Meta's hosted WhatsApp Cloud API (no local bridge).",
        "docs_url": "https://hermes-agent.nousresearch.com/docs/user-guide/messaging/whatsapp-cloud",
    },
    "relay": {
        "name": "Relay (experimental)",
        "description": "Generic relay adapter fronted by the Norual Relay connector.",
        "docs_url": "",
        "required_env": (),
    },
}
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in norual gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in norual gateway setup",
...
# hermes_cli/setup_hidden_env.py. Shared with the `norual setup gateway`
...
_TELEGRAM_ONBOARDING_USER_AGENT = f"NorualDashboard/{__version__}"
@dataclass
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Norual Agent").strip() or "Norual Agent"
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Norual appears
...
# the canonical ``norual auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.hermes/.anthropic_oauth.json`` — Norual-managed PKCE flow (what
    ...
    if hermes_creds and hermes_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "hermes_pkce",
            "source_label": f"Norual PKCE ({_get_hermes_oauth_file() if _get_hermes_oauth_file else None})",
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Norual even
    when they also have a separate Norual-managed PKCE login.
    """
    try:
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    Norual never claims a login state it can't verify.
    """
    return {
        "logged_in": False,
        "source": "copilot_cli",
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "norual auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "norual auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "norual auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "norual auth add minimax-oauth",
    ...
    },
    {
        ...
        # 127.0.0.1 callback.
        "flow": "device_code",
        "cli_command": "norual auth add xai-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "norual auth add anthropic",
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside Norual, so the disconnect
    ...
    user sees exactly what executes, and Norual then stops resolving the token.
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    if provider.get("flow") == "external":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside Norual — run the disconnect command to remove it."
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``norual model`` universe) — so any OAuth/external provider added as a
    ...
    #    in lockstep with the `norual model` universe (zero-edit for new plugins).
    try:
        from hermes_cli.provider_catalog import provider_catalog
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"norual auth add {d.slug}",
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    sync with the `norual model` picker; _OAUTH_OVERRIDES supplies per-provider
...
async def disconnect_oauth_provider(
    ...
    def _run():
        with _profile_scope(profile):
            ...
            # Anthropic clears only the Norual-managed PKCE file and auth-store entry.
...
# Guarded so norual web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Norual file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``norual auth add anthropic``.
    """
    from agent.anthropic_adapter import _get_hermes_oauth_file
    oauth_file = _get_hermes_oauth_file()
    payload = {
...
def _xai_device_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            # Mirror `norual auth add xai-oauth`: first credential may become
            ...
            # prior ``norual auth remove xai-oauth`` (mirrors auth_add_command
            # and the ``norual model`` re-login path in _login_xai_oauth).
...
def _codex_device_code_start_error(resp: Any) -> str:
    ...
    if "device" in lower and ("authori" in lower or "enable" in lower):
        message = (
            ...
            "account may need device-code authorization enabled before Norual "
...
def _session_db_read_probe_statements() -> tuple:
    ...
    after `norual update` until the first message forced a writable open.
    """
...
def _maybe_auto_archive_for_profile(profile: Optional[str]) -> None:
    ...
    The Desktop backend is spawned as ``norual serve`` — it runs neither the
...
def _prune_sessions(body: SessionPrune):
    """Delete ended sessions matching filters (mirrors `norual sessions prune`)."""
    has_window = (
...
def _gateway_intentionally_stopped(profile: Optional[str]) -> bool:
    ...
    commands (``norual gateway stop`` persists ``"stopped"``; start and
...
# servers managed here show up under `norual mcp list` and vice versa.  Secrets
...
def _run_dashboard_mcp_oauth(flow, cfg: dict) -> None:
    ...
    try:
        ...
        from tools.mcp_oauth import NorualTokenStorage, force_interactive_oauth
        ...
        try:
            ...
            with transaction, force_interactive_oauth(), dashboard_oauth_flow(flow):
                manager = get_manager()
                storage = NorualTokenStorage(flow.server_name)
...
# spawn the real `norual gateway <verb>` so behaviour matches the CLI exactly.
...
async def add_credential_pool_entry(body: CredentialPoolAdd):
    ...
    try:
        ...
        # (via DELETE below or `norual auth remove`) can seed again.
        # Mirrors the `norual auth add` behaviour in auth_commands.py.
...
async def remove_credential_pool_entry(provider: str, index: int):
    ...
    registry the CLI ``norual auth remove`` uses: each source cleans up its
...
def _profile_setup_command(name: str) -> str:
    ...
    return "norual setup" if name == "default" else f"{name} setup"
...
# (which attach to cua-driver's OWN identity, com.trycua.driver — not Norual,
...
#
# The endpoint spawns the same ``norual --tui`` binary the CLI uses, behind
...
def _resolve_chat_argv(
    ...
    Default: whatever ``norual --tui`` would run.  Tests monkeypatch this
    ...
    — the same propagation ``norual -p <name>`` performs. The in-process
    ...
    # Norual TUI child: build via the single spawn-env factory (profile-home
...
# ---------------------------------------------------------------------------
# /api/console — safe Norual Console command WebSocket.
#
# Unlike /api/pty, this endpoint never spawns a PTY, shell, or full Norual CLI
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from hermes_cli.console_engine import NorualConsoleEngine

        engine = NorualConsoleEngine(output_limit=_CONSOLE_OUTPUT_LIMIT)
            ...
            if command_id == command_generation:
                ...
                await _console_send(
                    ...
                    send_lock,
                    {
                        ...
                        "id": command_id,
                        "message": (
                            "Command timed out. Norual Console returned to the prompt."
                        ),
                        "command": line,
                    },
                )
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Norual inside WSL2 to use the dashboard's /chat "
...
def _render_active_theme_bootstrap_css() -> str:
    ...
    default Norual Teal canvas before the SPA flips the configured user
    theme into place.
...
def mount_spa(application: FastAPI):
    ...
    """
    # `norual serve` is the headless backend: it must NEVER serve the browser
    ...
    if _headless or not WEB_DIST.exists():
        _msg = (
            "Headless backend (norual serve): web UI disabled — use "
            "`norual dashboard` for the browser UI."
            if _headless
    ...
    def _serve_index(prefix: str = ""):
        ...
        # Without this the SPA paints the default Norual Teal canvas, then
    ...
    # (the MC Pages app), not the Norual backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Norual Teal",         "description": "Classic dark teal — the canonical Norual look"},
    {"name": "default-large", "label": "Norual Teal (Large)", "description": "Norual Teal with bigger fonts and roomier spacing"},
...
def _discover_dashboard_plugins() -> list:
    ...
    # installed in the norual root (``~/.hermes/plugins``). Scan the default
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if cached_result is False:
                        auth_required = True
                        auth_command = f"norual auth {name}"
                        break
            except Exception:
                pass
...
# spawn (and any script wrapping ``norual serve``) cannot tell "port occupied"
...
def _report_port_in_use(host: str, port: int) -> None:
    ...
    print(
        ...
        "'norual serve' / 'norual dashboard' backend or the Norual gateway. "
...
def start_server(
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # Each provider plugin that ships with Norual Agent exposes a
            ...
            _fix_hint = (
                ...
                "  • OAuth: run `norual dashboard register` (Nous Portal) or "
            ...
            try:
                ...
                if _has_creds and (set(_disabled) & _BASIC_AUTH_PLUGIN_KEYS):
                    _fix_hint = (
                        ...
                        "`norual plugins enable basic`), then restart the "
                        "dashboard.\n\n"
                    ) + _fix_hint
            except Exception:
                pass
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            # for standalone `norual serve` (no HERMES_PARENT_PID env).
            ...
            if headless:
                ...
                print(f"  Norual backend listening on {host}:{actual_port}", flush=True)
            else:
                print(f"  Norual Web UI → http://{host}:{actual_port}")
