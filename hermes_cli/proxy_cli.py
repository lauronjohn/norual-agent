"""CLI handlers for ``norual egress ...``.
...
The top-level command is ``norual egress``.  Note that the inbound OAuth
reverse-proxy command (``norual proxy``) lives elsewhere in
...
def register_cli(parent_parser: argparse.ArgumentParser) -> None:
    ...
    ``norual egress`` parser.
    """

    # dest='egress_command' — keeps this subparser tree disjoint from the
    # inbound OAuth ``norual proxy`` subparser (which uses dest='proxy_command').
    ...
    setup.add_argument(
        ...
        help="Do not restart a running daemon after setup; you'll need to run "
             "`norual egress restart` yourself for changes to take effect.",
    )
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    if args.from_bitwarden:
        ...
        if not bw_cfg.get("enabled"):
            ...
            console.print(
                "  Run `norual secrets bitwarden setup` first, or omit "
                "--from-bitwarden."
            )
            return 1
        try:
        ...
        # to loading that file so `norual egress setup` finds the same keys the
    ...
    if rotate and existing:
        ...
        if _sys.stdin.isatty():
            console.print(
                ...
                "tokens in every running Norual sandbox.  They will start "
    ...
    # `norual egress reload` apply future ruleset changes without a
    ...
    # rotation), running `norual egress setup` again WITHOUT
    ...
    if do_restart:
        try:
            ...
            console.print(
                "  Run [cyan]norual egress start[/cyan] manually before "
        ...
        console.print(
            ...
            "changed.  Run [cyan]norual egress restart[/cyan] (or "
    ...
    console.print(
        "  Start:   [cyan]norual egress start[/cyan]\n"
        "  Restart: [cyan]norual egress restart[/cyan]  (after any re-setup)\n"
        "  Reload:  [cyan]norual egress reload[/cyan]   (apply ruleset edits "
        "in-place, no restart)\n"
        "  Status:  [cyan]norual egress status[/cyan]\n"
        "  Stop:    [cyan]norual egress stop[/cyan]\n"
        "  Disable: [cyan]norual egress disable[/cyan]"
    )
    return 0
...
def cmd_start(args: argparse.Namespace) -> int:
    ...
    if not proxy_cfg.get("enabled"):
        console.print(
            "[yellow]proxy.enabled is false — run `norual egress setup` "
            "first.[/yellow]"
        )
        return 1
    ...
    if credential_source == "bitwarden" and not refresh_bw:
        if bool(proxy_cfg.get("allow_env_fallback", False)):
            ...
            console.print(
                ...
                "back to env credentials with `norual egress setup "
    ...
    if refresh_bw:
        ...
        if not os.environ.get(bw_access_env, "").strip():
            ...
            console.print(
                ...
                "`norual egress setup --no-bitwarden` to switch back to "
        ...
        if not (bw_cfg or {}).get("project_id"):
            ...
            console.print(
                "  Run `norual secrets bitwarden setup` to configure the "
                "project, or switch back via `norual egress setup "
                "--no-bitwarden`."
            )
            return 1
...
def cmd_reload(args: argparse.Namespace) -> int:
    ...
    ``norual egress restart`` instead: the daemon reads real credentials
    ...
    console.print(
        ...
        "still need `norual egress restart` — the daemon reads real "
...
def format_status_text(*, show_tokens: bool = False) -> str:
    ...
    if bool(proxy_cfg.get("enabled")) and not status.configured:
        lines.extend(["", "Next: run `norual egress setup` to mint tokens and write proxy.yaml."])
    elif bool(proxy_cfg.get("enabled")) and not (status.pid and status.listening):
        lines.extend(["", "Next: run `norual egress start` before launching Docker sandboxes."])
...
def cmd_disable(args: argparse.Namespace) -> int:
    ...
    if ip.get_status().pid is not None:
        console.print(
            ...
            "[cyan]norual egress stop[/cyan] if you want it down too."
        )
    return 0
...
def cmd_config(args: argparse.Namespace) -> int:
    ...
    if status.config_path is None:
        console.print(
            "[yellow](no config generated — run `norual egress setup`)[/yellow]"
        )
        return 1
...
def _load_env_file_into_environ() -> int:
    ...
    ``norual egress setup`` discovers providers by reading ``os.environ``, but
