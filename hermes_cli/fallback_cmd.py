"""
norual fallback — manage the fallback provider chain.
...
Subcommands:
  norual fallback [list]   Show the current fallback chain (default when no subcommand)
  norual fallback add      Pick provider + model via the same picker as `norual model`,
                           then append the selection to the chain
  norual fallback remove   Pick an entry to delete from the chain
  norual fallback clear    Remove all fallback entries
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        ...
        # `norual model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  norual fallback add")
        print()
        return
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `norual model`, then append the selection to the chain."""
    ...
    print("  `norual model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `norual fallback list` to view, or `norual fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``norual fallback [subcommand]``."""
