"""``norual logs`` — view and filter Norual log files.
...
Usage examples::
    ...
    norual logs                    # last 50 lines of agent.log
    norual logs -f                 # follow agent.log in real time
    norual logs errors             # last 50 lines of errors.log
    norual logs gateway -n 100    # last 100 lines of gateway.log
    norual logs gui -f            # follow gui.log (dashboard/pty/ws)
    norual logs desktop -f        # follow desktop.log (Electron app boot/backend)
    norual logs --level WARNING    # only WARNING+ lines
    norual logs --session abc123   # filter by session ID substring
    norual logs --component tools  # only tool-related lines
    norual logs --since 1h         # lines from the last hour
    norual logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    if not log_path.exists():
        ...
        print("(Logs are created when Norual runs — try 'norual chat' first)")
        sys.exit(1)
...
def list_logs() -> None:
    ...
    if not found:
        print("  (no log files yet — run 'norual chat' to generate logs)")
