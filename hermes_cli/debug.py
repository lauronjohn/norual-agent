"""``norual debug`` debug tools for Norual Agent.

Currently supports:
    norual debug share    Upload debug report (system info + logs) to a
...
_REDACTION_BANNER = (
    "[norual debug share: log content redacted at upload time. "
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``norual debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``norual debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `norual debug share` "
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``norual debug share`` invocation added ~20 MB of resident Python
    ...
    ``norual debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----NorualDebugBoundary9f3c"
...
# desktop app talks to a remote/docker/SSH backend, `norual debug share` runs
...
_CLIENT_SIDE_LOGS = {
    "desktop": (
        "written by Norual Desktop on the machine running the app, not by this "
...
def _capture_dump() -> str:
    """Run ``norual dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``norual dump`` is run
        internally.
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``norual debug share`` (CLI) and the
    ...
    if redact:
        logger.info(
            "norual debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `norual debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print("To delete now:  norual debug delete <url>")

    print("\nShare these links with the Norual team for support.")
...
⚠️  --nous: This uploads your debug bundle to Nous-INTERNAL storage (AWS S3),
  ...
  • System info (OS, Python/Norual version, provider, which API keys are
...
def _run_debug_share_nous(args, *, log_lines: int, redact: bool) -> None:
    """Handle ``norual debug share --nous``: upload the bundle to Nous-S3.
    ...
    if redact:
        logger.info(
            "norual debug share --nous: applied force-mode redaction before upload"
        )
    ...
    try:
        ...
        print(
            ...
            "provisioned.\n"
            "Run `norual debug share --local` to print the report instead, "
            "or `norual debug share` to upload to a public paste service.\n",
            file=sys.stderr,
        )
        sys.exit(1)
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: norual debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'norual debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``norual debug`` call.
    ...
    # best-effort — any failure is swallowed so ``norual debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: norual debug <command>")
        print()
        print("Commands:")
