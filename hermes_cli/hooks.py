"""norual hooks — inspect and manage shell-script hooks.

Usage::

    norual hooks list
    norual hooks test <event> [--for-tool X] [--payload-file F]
    norual hooks revoke <command>
    norual hooks doctor
...
def hooks_command(args) -> None:
    """Entry point for ``norual hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: norual hooks {list|test|revoke|doctor}")
        print("Run 'norual hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs and not outbound:
        ...
        print("See `norual hooks --help` or")
                    ...
                    if entry and entry.get("approved_at"):
                        ...
                        if mtime_now and mtime_at and mtime_now > mtime_at:
                            print(
                                ...
                                f"run `norual hooks doctor` to re-validate"
                            )
            print()
...
# stdin a script sees under `norual hooks test` and `norual hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Norual wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `norual hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `norual hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `norual hooks doctor`.")
