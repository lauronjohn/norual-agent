"""Unified provider-credential lifecycle across every store Norual reads.
...
def purge_env_credential_references(
    ...
    # Make the removal sticky the same way `norual auth remove` does: a
...
def save_provider_env_credential(env_var: str, value: str) -> Dict[str, Any]:
    ...
    re-add through the UI behaves like ``norual auth add``.
    """
