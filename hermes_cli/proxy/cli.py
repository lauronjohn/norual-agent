"""CLI handlers for the ``norual proxy`` subcommand."""
...
def _print_aiohttp_missing() -> None:
    print(
        "norual proxy requires aiohttp. Run `norual setup` to install it.",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"norual auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Norual proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Norual proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: norual proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``norual proxy <subcommand>``."""
    ...
    print(
        "norual proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  norual proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
        "      Run the proxy in the foreground.\n"
        "  norual proxy status\n"
        "      Show which upstream adapters are ready.\n"
        "  norual proxy providers\n"
