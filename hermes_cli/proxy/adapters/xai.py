...
# xAI's public API is OpenAI-compatible for the endpoints Norual commonly
# uses. The Responses endpoint is included because Norual' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Norual-managed OAuth credentials."""

    auth_hint = "norual auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`norual auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`norual auth reset xai-oauth` or re-authenticate with "
                    "`norual auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `norual auth add xai-oauth --type oauth`."
            )
