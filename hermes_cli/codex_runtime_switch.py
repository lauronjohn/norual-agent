...
Toggles `model.openai_runtime` between "auto" (= chat_completions, Norual'
...
def apply(
    ...
    *,
    ...
    if new_value == "codex_app_server":
        ...
        # Auto-migrate Norual' MCP servers + Codex's installed curated
        # plugins into ~/.codex/config.toml so the spawned codex subprocess
        # sees the same tool surface AND can call back into Norual for
        ...
        try:
            ...
            # Permissions + Norual tool callback are always-on production
            ...
            if "hermes-tools" in mig_report.migrated:
                msg_lines.append(
                    "Norual tool callback registered: codex can now use "
                ...
                msg_lines.append(
                    ...
                    "only on the default Norual runtime — they need the "
                    "agent loop context.)"
                )
        ...
        msg_lines.append(
            ...
            "Norual tools available via MCP callback)."
        )
        msg_lines.append(
        ...
        msg_lines.append("OpenAI/Codex turns will use the default Norual runtime.")
