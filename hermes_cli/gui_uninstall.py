"""
Norual Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``norual desktop`` / ``norual gui``)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Norual.app`` or ``~/Applications/Norual.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Norual`` (NSIS per-user)
...
the app name ("Norual"), separate from ``$HERMES_HOME``:
  - macOS:   ``~/Library/Application Support/Norual``
  - Windows: ``%APPDATA%\\Norual``
  - Linux:   ``$XDG_CONFIG_HOME/Norual`` (default ``~/.config/Norual``)
...
``norual uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def desktop_userdata_dir() -> Path:
    ...
    Mirrors Electron's ``app.getPath('userData')`` for an app named "Norual"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Norual"
...
def source_built_gui_artifacts(hermes_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``norual desktop`` inside the checkout.
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Norual" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Norual.app"),
            home / "Applications" / "Norual.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Norual).
            local_base / "Programs" / "Norual",
        ...
        if program_files:
            ...
            paths.append(Path(program_files) / "Norual")
    else:
        ...
        paths += [
            # The launcher entry `norual desktop` installs. Its icon lives
            # in the checkout, not in the installed app.
            desktop_entry_path(),
            # Some packaged builds emit this casing.
            data_base / "applications" / "Norual.desktop",
        ]
    return paths
...
def uninstall_gui(hermes_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    if sys.platform.startswith("linux"):
        ...
        # Reindex so Norual disappears from the launcher.
        try:
