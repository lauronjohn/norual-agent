"""
Unified self-relaunch for Norual CLI.

Preserves critical flags (--tui, --dev, --profile, --model, etc.) across
process replacement so that ``norual sessions browse`` or post-setup relaunch
...
def relaunch(
    ...
    *,
    ...
    """Replace the current process with a fresh norual invocation.
    ...
    relaunch contract wants: "run norual again as if the user had typed
    the new argv".
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    That's functionally equivalent — the user sees "norual exited, then
    new norual started" — just with two PIDs in play instead of one.
    """
