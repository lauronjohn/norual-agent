...
Scope of the repair is deliberately narrow. Norual only upgrades an npm that
...
and their other projects; Norual never modifies those. When the failing npm is
one of those foreign installs, Norual instead provisions its own managed Node
...
def managed_npm_prefix(npm: str | os.PathLike[str] | None) -> Path | None:
    """Return the Norual-managed Node root *npm* lives in, else ``None``.
...
def upgrade_managed_npm(
    ...
    *,
    ...
    if not quiet:
        print(
            f"→ Upgrading Norual-managed npm to satisfy {npm_range}…",
            flush=True,
        )
    ...
    if managed_node_tree_in_use():
        if not quiet:
            print(
                "  ⚠ deferred: the Norual-managed Node.js tree is in use by a "
...
def _print_manual_fix(npm: str, npm_range: str, actual: str | None) -> None:
    ...
    print(
        ...
        f"  Resolved npm: {npm}\n"
        "  Norual could not provision its own Node.js runtime and never\n"
...
def _provision_managed_npm(npm_range: str | None, *, quiet: bool = False) -> str | None:
    """Provision a Norual-managed Node tree and return a satisfying npm.
    ...
    if not quiet:
        print(
            "→ Provisioning a Norual-managed Node.js runtime "
...
def maybe_repair_npm_engine(
    ...
    *output* is the combined stdout/stderr of the npm command that just failed.
    ...
    the same *npm* after an in-place upgrade of a Norual-managed install, or
    ...
    if prefix is not None:
        # Norual owns this npm — upgrade it in place. Only an npm-range
