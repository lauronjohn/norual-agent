...
async def mcp_oauth_callback(
    ...
    if flow is None:
        return HTMLResponse("<h1>OAuth flow expired</h1><p>Return to Norual and try again.</p>", status_code=404)
    try:
    ...
    if error:
        return HTMLResponse("<h1>Authorization failed</h1><p>Return to Norual for details.</p>", status_code=400)
    return HTMLResponse("<h1>Authorization received</h1><p>You can close this tab and return to Norual.</p>")
...
async def list_mcp_catalog(profile: Optional[str] = None):
    ...
    `norual mcp catalog` / `norual mcp install` read.  ``profile`` scopes
