...
``norual gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``norual gateway start/stop/restart``).
    """
    ...
    # `norual gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    detection always fails. Since every Norual runtime call inside the
    container drops to norual via ``s6-setuidgid``, that silent failure
...
# active (set via the ``norual -p <profile>`` flag before the call). This
...
#
# Per-profile gateways are registered dynamically when `norual profile create`
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the norual user.
    ...
    (the ``hermes`` user — UID 10000 — running every Norual runtime
    ...
    is dispatched as the norual user.
    ...
    with norual ownership before triggering ``s6-svscanctl -a``,
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        try:
            ...
            # Running as the norual user already — directory is hermes-
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``norual -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`norual profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    def _render_run_script(
        ...
        The script:
          ...
          4. Drops to the norual user and exec's
             ``norual -p <profile> gateway run`` (or just ``hermes
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``norual gateway
        ...
        # (a stray ``norual gateway run`` from a shell, an agent action, or
        ...
        if profile == "default":
            gateway_cmd = "norual gateway run --replace"
        else:
            gateway_cmd = f"norual -p {shlex.quote(profile)} gateway run --replace"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid norual {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``norual logs`` reads and what
        ...
        return (
            ...
            # Create the leaf and clear a stale s6-log lock as norual when
            ...
            f'if [ "$(id -u)" = 0 ]; then\n'
            f'  s6-setuidgid norual mkdir -p "$log_dir"\n'
            f'  s6-setuidgid norual rm -f "$log_dir/lock"\n'
            f'else\n'
            f'  mkdir -p "$log_dir"\n'
            f'  rm -f "$log_dir/lock"\n'
            f'fi\n'
            ...
            f'exec s6-setuidgid norual s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``norual gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``norual -p <profile> gateway start``.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with norual ownership
            ...
            # the norual user) won't hit EACCES on root-owned 0700
