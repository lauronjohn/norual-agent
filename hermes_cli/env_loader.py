"""Helpers for loading Norual .env files consistently across entrypoints."""
...
# Used by setup / `norual model` flows to label detected credentials so
...
def _known_hermes_env_keys() -> set[str]:
    """Return the combined set of known Norual env-var keys.
...
# Behavioral routing keys a parent Norual process injects into child env and
...
def _clear_known_keys_missing_from_dotenv(path: Path) -> None:
    """Remove inherited profile-managed Norual keys absent from ``.env``.
    ...
    parent Norual process injects and that silently change *which provider
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `norual setup` (or edit the "
...
def load_hermes_dotenv(
    *,
    ...
    """Load Norual environment files with user config taking precedence.
    ...
    if user_env.exists():
        ...
        # Mirror reload_env() known-key cleanup so inherited Norual keys
    ...
    # 2. A fresh ``norual update`` retry just completed a deferred dependency
    ...
    # `norual setup` before the user switched terminal.backend in config.yaml)
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    to locate bootstrap tokens) but BEFORE the rest of Norual reads
    ...
    # a generic dict entry must not force crypto load on every norual launch.
    ...
    if report.applied_any:
        ...
        _sanitize_loaded_credentials()
        # Remember where each var came from so setup / `norual model`
