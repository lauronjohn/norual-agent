"""
Doctor command for norual CLI.

Diagnoses issues with Norual Agent setup.
"""
...
_DHH = display_hermes_home()  # user-facing display path (e.g. ~/.norual or ~/.hermes/profiles/coder)
...
def _sqlite_upgrade_hint(install_method: str | None = None) -> str:
    ...
    if method == "docker":
        ...
        action = f"run `{command}`, then recreate all Norual containers"
...
def _hermes_database_paths(hermes_home: Path) -> list[tuple[str, Path]]:
    """Return (display name, path) pairs for Norual-managed SQLite databases."""
...
def _report_database_journal_modes(
    ...
    try:
        ...
        check_warn(f"Could not list Norual databases: {exc}")
        return
    exposed = []
...
def _render_state_db_stats(stats: dict, holders=None) -> list:
    ...
    if isinstance(deferral, dict):
        ...
        lines.append((
            ...
            "(stop the listed processes, then run 'norual sessions "
    ...
    if logical is not None and logical > STATE_DB_SIZE_WARN_BYTES:
        ...
        if stats.get("fts_rebuild_pending") or legacy_trigram:
            detail += (
                "; run 'norual sessions optimize-storage' offline "
...
def collect_relay_plugin_cutover_findings(
    ...
    """Return actionable findings for the removed Norual Relay plugin."""
...
def _check_version_consistency(issues: list[str]) -> None:
    ...
    other, leaving ``norual --version`` reporting a stale version while
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            "Re-sync version files (e.g. run 'norual update', or set "
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-norual and dashboard static services are up
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `norual profile create <name>`")
        return
...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    ...
    if not should_fix:
        ...
        if issues is not None:
            issues.append(
                "Repair the CA bundle: run `norual doctor --fix`, or "
...
def check_macos_tcc_grants() -> None:
    ...
    if "cdhash" in dr.lower():
        check_warn(
            ...
            "Run `norual update` to get the stable identifier-pinned signing "
            "identity, then re-grant permissions once.",
        )
        return
    if "certificate" in dr.lower():
        # Certificate-anchored DR (norual desktop --setup-tcc-identity, or a
        ...
        check_ok(
            ...
            "anchor, see `norual desktop --setup-tcc-identity`)",
        )
    check_info(
        ...
        "fully quit & relaunch Norual once."
    )
...
def _desktop_app_bundle() -> Path | None:
    ...
    (``apps/desktop/release/mac-<arch>/Norual.app``) — the only layout whose
    ...
    ``_desktop_packaged_executable``'s selection. ``/Applications/Norual.app``
    is deliberately not probed: it is the separately-signed Norual-Setup
    ...
    candidates = [p for p in release_dir.glob("mac*/Norual.app") if p.is_dir()]
    if not candidates:
        return None
...
def check_macos_tcc_anchor(should_fix: bool = False) -> None:
    ...
    try:
        ...
        check_warn(
            ...
            "Run `norual doctor --fix` to pin the interpreter at a stable path.",
        )
...
def run_doctor(args):
    ...
    # Handle `norual doctor --ack <id>` as a fast path. Persist the ack and
    ...
    print(color("│                 🩺 Norual Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`norual doctor --ack {hit.advisory.id}`."
                )
    ...
    try:
        ...
        if is_sqlite_wal_reset_vulnerable():
            # Warn-only: Norual already refuses to enable WAL on fresh DBs.
    ...
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'norual setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'norual setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'norual setup' to create one")
                issues.append("Run 'norual setup' to create .env")
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'norual config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
                ...
                try:
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.hermes/.env or run 'norual setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'norual setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'norual config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'norual setup' to migrate config")
                else:
                    issues.append("Run 'norual doctor --fix' or 'norual setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'norual doctor --fix'")
        except Exception:
            pass
        ...
        try:
            ...
            if drift:
                check_warn(
                    ...
                    "(stale ghost from an earlier `norual setup` run)",
                )
                if should_fix:
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Norual' own device-code flow — the Codex CLI is
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Norual a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Norual Agent Persona\n\n"
                "<!-- Edit this file to customize how Norual communicates. -->\n\n"
                "You are Norual, a helpful AI assistant.\n",
                encoding="utf-8",
            )
        ...
        try:
            ...
            if _write_reason is not None:
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db FTS write corruption — run 'norual doctor --fix' "
                        "(or 'norual sessions repair') to rebuild the FTS index"
                    )
        except Exception as e:
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'norual doctor --fix' "
                        "(or 'norual sessions repair') to recover hidden sessions"
                    )
            else:
                check_warn(f"{_DHH}/state.db exists but has issues: {e}")

        # Health/stats snapshot (#statedb-visibility): a multi-GB state.db
        # with a runaway WAL was previously invisible to every Norual
                ...
                if _kind == "warn":
                    ...
                    if "auto_prune" in _detail:
                        issues.append(
                            ...
                            "in config.yaml"
                            + (
                                " and run 'norual sessions optimize-storage' "
        ...
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'norual doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        if _venv_bin is None:
            check_warn(
                ...
                "(norual not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/norual → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/norual points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/norual → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/norual — run 'norual doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/norual exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/norual not found",
                    "(norual command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/norual → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/norual symlink — run 'norual doctor --fix'")
    ...
    if _safe_which("node"):
        ...
        if _resolved_ab and _is_npx_agent_browser_sentinel(_resolved_ab):
            ...
            if should_fix:
                ...
                # `norual update` does, so a session's first browser call
            ...
            # `norual update` wiped node_modules (issue #48521).
            check_warn(
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `norual doctor` invocation.
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a Norual one.
                        check_info(
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'norual config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``norual doctor``.
    ...
    # never the legitimate source for `norual doctor`.
    ...
    try:
        ...
        if api_disabled or web_not_ready:
            issues.append("Run 'norual setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: norual skills list)")
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: norual memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: norual memory setup",
                    "No Honcho API key — run 'norual memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run norual memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: norual memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: norual memory setup")
        except Exception as _e:
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "norual -p" in content:
                            _m = _re.search(r"norual -p (\S+)", content)
    ...
    # `norual doctor --live` (real network calls; bounded + read-only).
    try:
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'norual doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
