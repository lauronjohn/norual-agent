...
This means ``norual dashboard --host 0.0.0.0`` is safe to run on a LAN:
...
class UpdateTaskBody(BaseModel):
    ...
    # transitions to 'done'. Dashboard parity with ``norual kanban
...
def list_diagnostics(
    ...
    AND for ``norual kanban diagnostics`` which shells to this
...
def reclaim_task_endpoint(
    ...
    ``norual kanban reclaim <task_id> --reason ...``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def specify_task_endpoint(
    ...
    it to ``todo``. Maps 1:1 to ``norual kanban specify <task_id>``.
...
def reassign_task_endpoint(
    ...
    Maps 1:1 to ``norual kanban reassign <task_id> <profile> [--reclaim]``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def _active_profile_name() -> str:
    """Return the current Norual profile name for notify-sub ownership."""
    try:
...
def model_options():
    ...
    dropdown can never offer a model/provider pair the rest of Norual
...
def list_kanban_projects():
    """List first-class Norual projects for board scoping.
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    Maps 1:1 to ``norual profile describe <name> --auto``. Non-OK
...
def decompose_task_endpoint(
    ...
    1:1 to ``norual kanban decompose <task_id>``.
