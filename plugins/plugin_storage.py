...
``<norual home>/plugins/<name>/``. That tree is the plugin *install* dir —
``norual plugins remove`` deletes it and ``norual plugins update`` git-pulls
...
``<norual home>/plugin-data/<name>/``, owned by the user, untouched by
...
through ``agent.secret_scope`` / ``.env`` like everywhere else in Norual.
...
# Mirrors the plugin-name shape `norual plugins install` accepts. Anything
...
def plugin_data_dir(name: str) -> Path:
    ...
    ``<norual home>/plugin-data/<name>/`` — survives plugin update and
