...
Handles: norual honcho setup | status | sessions | map | peer
"""
...
def cmd_sync(args) -> None:
    ...
    Scans all Norual profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'norual honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'norual honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `norual update` -- no output, no exceptions.
    """
    try:
        from hermes_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Norual profile."""
...
def _ensure_sdk_installed() -> bool:
    ...
    # data-volume target instead of writing to /opt/norual (NS-605).
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Norual persistent cross-session memory.")
    ...
    if not is_local:
        ...
        if use_device:
            ...
            try:
                ...
                print("\n  Cancelled. Re-run 'norual honcho setup' to try again.\n")
                return
            except (AuthorizationTimeout, DeviceCodeExpired):
                print("\n  Device code expired before approval.")
                print("  Re-run 'norual honcho setup' to get a new code.\n")
                return
            except AccessDenied:
                print("\n  Sign-in was denied on the approval page.")
                print("  Re-run 'norual honcho setup' to retry, or choose an API key instead.\n")
                return
            ...
            if not cfg.get("apiKey"):
                ...
                print("  Run 'norual honcho setup' again once you have a key.\n")
                return
    ...
    # These keys only affect the Norual GATEWAY (Telegram/Discord/Slack/...),
    ...
    if gw_platforms is None:
        ...
        run_mapping = _prompt(
            "Running the Norual gateway (Telegram/Discord/etc.)? (y/N)",
            default="n",
    ...
    try:
        ...
        print("  Run: norual config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    norual honcho status     -- show full config")
    print("    norual honcho mode       -- change recall/observation mode")
    print("    norual honcho tokens     -- tune context and dialectic budgets")
    print("    norual honcho peer       -- update peer names")
    print("    norual honcho map <name> -- map this directory to a session name\n")
...
def _active_profile_name() -> str:
    """Return the active Norual profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: norual honcho setup\n")
        return
    ...
    if not cfg:
        ...
        try:
            ...
            if _env_cfg.api_key or _env_cfg.base_url:
                ...
                print("  Run 'norual honcho setup' to configure.\n")
                return
        except Exception:
            print(f"  No Honcho config found at {active_path}")
            print("  Run 'norual honcho setup' to configure.\n")
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: norual honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        ...
        print(f"  AI peer:     {ai}")
        print("    Norual' identity in Honcho. Seed with 'norual honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print("\n  Set with: norual honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print("\n  Set with: norual honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ...
        print("    AI-to-AI inference. Norual asks Honcho's AI peer a question")
        ...
        print("\n  Set with: norual honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'norual honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'norual honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    norual honcho identity --show        — show both peer representations")
        print("    norual honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Norual + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Norual\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Norual persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  norual honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'norual honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'norual honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  norual honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print("  (Norual calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    norual honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'norual honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Norual, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'norual honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    norual honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  norual honcho identity <path/to/SOUL.md>")
    ...
    print("    Norual:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Norual:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Norual:   Honcho observes every message and updates representations")
    ...
    print("    Norual:   per-session by default — each run gets its own session")
    print("              Map a custom name:  norual honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. norual honcho setup              — configure API key (required)")
        print("  2. norual honcho migrate            — re-run this walkthrough")
    else:
        print("  1. norual honcho status             — verify Honcho connection")
        print("  2. norual                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. norual honcho identity --show    — verify AI peer representation")
        print("  4. norual honcho tokens             — tune context and dialectic budgets")
        print("  5. norual honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'norual memory setup'...\n")
...
def register_cli(subparser) -> None:
    """Build the ``norual honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``norual honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to norual memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Norual Honcho",
    )
