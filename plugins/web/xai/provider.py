...
``{title, url, description, position}`` rows every other Norual web
provider produces.
...
prefers Norual-managed xAI Grok OAuth (via ``norual auth``) and falls back
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``norual tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `norual auth` to sign in with "
