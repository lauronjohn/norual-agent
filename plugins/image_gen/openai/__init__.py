...
three virtual model IDs so the ``norual tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `norual tools` → Image "
                    "Generation → OpenAI to configure, or `norual setup` "
                    "to add the key."
                ),
