"""
IRC Platform Adapter for Norual Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Norual agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Norual Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Norual Agent shutting down")
...
def check_requirements() -> bool:
    ...
    # norual setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `norual gateway setup` flow for the IRC platform.
    ...
    print_info("Connect Norual to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #norual — comma-separate for multiple)",
    ...
    print_info("Restart the gateway for changes to take effect: norual gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Norual Agent (cron)")
