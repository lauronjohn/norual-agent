"""
LINE Messaging API platform adapter for Norual Agent.
...
to Norual Agent. It deliberately ports the *strongest* idea from each into
...
def is_connected(config) -> bool:
    """Surface in ``norual status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``norual status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``norual setup line``.
