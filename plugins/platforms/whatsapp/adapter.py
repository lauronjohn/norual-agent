...
def _is_allowed_bridge_path(url: str) -> bool:
    ...
    known Norual media cache directory.
...
def check_whatsapp_requirements() -> bool:
    ...
    """
    # Prefer Norual-managed Node/npm so Windows installs are not broken by a
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not check_whatsapp_requirements():
            ...
            self._set_fatal_error(
                ...
                "Node.js is not installed — install Node.js and re-run `norual gateway`.",
                retryable=False,
            )
            return False
        ...
        if not creds_path.exists():
            logger.warning(
                ...
                "Pair from the dashboard or run `norual whatsapp`; remove "
                "WHATSAPP_ENABLED from your .env to disable.",
                self.name, creds_path,
            )
            self._set_fatal_error(
                "whatsapp_not_paired",
                "WhatsApp enabled but not paired — pair from the dashboard or run `norual whatsapp`.",
                retryable=False,
            )
            return False
        ...
        try:
            ...
            # `norual update` bumps the Baileys pin).  The stamp file records
            ...
            if not _deps_fresh:
                ...
                # Norual-managed portable Node before falling back to PATH.
                ...
                try:
                    ...
                    if install_result.returncode != 0:
                        ...
                        self._set_fatal_error(
                            ...
                            f"WhatsApp bridge npm install failed. Run `cd {bridge_dir} && {_npm_bin} install` manually, then restart `norual gateway`.",
                            retryable=False,
                        )
                        return False
                    ...
                    self._set_fatal_error(
                        "whatsapp_npm_install_failed",
                        f"WhatsApp bridge npm install failed ({e}). Run `cd {bridge_dir} && {_npm_bin} install` manually, then restart `norual gateway`.",
                        retryable=False,
                    )
                    return False
                    ...
                    async with session.get(
                        ...
                        if resp.status == 200:
                            ...
                            if bridge_status == "connected":
                                ...
                                # `norual update`, so without this check it
            ...
            if data.get("status") != "connected":
                ...
                for attempt in range(15):
                    ...
                    print(f"[{self.name}]   If session expired, re-pair: norual whatsapp")
...
def _is_connected(config) -> bool:
    ...
    would make WhatsApp always show as "configured" in ``norual setup`` even
