"""SimpleX Chat platform adapter (Norual plugin).
...
This adapter ships as a Norual platform plugin under
``plugins/platforms/simplex/``. The Norual plugin loader scans the
...
discoverable and ``norual setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Norual — no
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``norual setup gateway`` → SimpleX.
