...
Credential storage mirrors every other Norual channel:
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a norual dependency
...
# registers Norual as its own client_id.
...
# Default name of the project Norual provisions for the operator.
DEFAULT_PROJECT_NAME = "Norual Agent"
...
def _auth_json_path() -> Path:
    """Resolve ``~/.hermes/auth.json`` honouring the active Norual profile."""
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `norual photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `norual photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `norual photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `norual photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `norual photon setup`)"
