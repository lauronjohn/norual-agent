...
def _needs_server_members_intent(
    ...
    """Return True when Norual must request Discord's Server Members intent.
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _handle_bot_task_done(self, task: asyncio.Task) -> None:
        ...
        marked running, the Discord websocket is dead while the Norual gateway
    ...
    def _classify_connect_exception(self, error: Exception) -> tuple:
        ...
        if _is("PrivilegedIntentsRequired"):
            # Name the exact intents Norual requested (#79430): Message
    ...
    async def _run_missed_message_backfill(self) -> None:
        ...
        that lack a substantive non-outage Norual response. Emoji-only acks are
    ...
    async def _should_backfill_discord_message(self, message: Any) -> bool:
        """Return True when a recent Discord message still needs Norual work."""
    ...
    def _is_down_notice_content(self, content: str) -> bool:
        """Recognize only explicit Norual/gateway outage notices."""
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields Norual manages."""
    ...
    def _is_pairing_approved_user(self, user_id: str) -> bool:
        """True when the Discord user has an explicit Norual pairing grant."""
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your Norual session")
        ...
        @tree.command(name="status", description="Show Norual session status")
        ...
        @tree.command(name="stop", description="Stop the running Norual agent")
        ...
        @tree.command(name="update", description="Update Norual Agent to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the Norual gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a Norual session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to Norual in the thread",
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        try:
            ...
            cmd = discord.app_commands.Command(
                name="skill",
                description="Run a Norual skill",
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new Norual session in the thread
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by Norual: **{name}**"
    ...
    def _derive_auto_thread_name(self, content: str) -> str:
        ...
        Norual has an LLM-generated session title and can safely rename
        ...
        thread_name = content[:80] if content else "Norual"
        if len(content) > 80:
            ...
            try:
                ...
                try:
                    seed_msg = await message.channel.send(
                        f"\U0001f9f5 Thread created by Norual: **{thread_name}**"
                    )
                    thread = await seed_msg.create_thread(
                        name=thread_name,
    ...
    async def rename_thread(
        ...
        *,
        ...
        if edit is None:
            return False
        try:
            await edit(name=cleaned, reason="Norual semantic session title")
            logger.info(
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 Norual handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
                name=thread_name,
                auto_archive_duration=1440,
                reason=reason,
    ...
    async def send_exec_approval(
        ...
        try:
            ...
            prompt_prefix = (
                ...
                "Do you want Norual to run this command?\n\n"
                "**Requested command:**\n```bash\n"
            )
            if smart_denied:
    ...
    async def send_clarify(
        ...
        try:
            ...
            content = self._self_contained_prompt_content(
                "❓ **Norual needs your input**", str(question or "").strip(),
                tail=clarify_tail,
            )
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``norual update --gateway``
                ...
                if thread:
                    ...
                    try:
                        await message.channel.send(
                            "⚠️ Norual could not create a Discord thread for "
...
def _component_check_auth(
    ...
    the pairing store (``norual pairing approve``).
    ...
    # so users approved via ``norual pairing approve`` can interact with
...
def _define_discord_view_classes() -> None:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``norual update`` prompts.
...
# is not in this process (e.g. ``norual cron`` running standalone) and no live
...
def register(ctx) -> None:
    """Plugin entry point — called by the Norual plugin system."""
    ctx.register_platform(
        ...
        install_hint="Run `norual setup` to install Discord support.",
