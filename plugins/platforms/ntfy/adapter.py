"""ntfy platform adapter (Norual plugin).
...
``plugins/platforms/ntfy/``. The Norual plugin loader scans the
...
async def _standalone_send(
    ...
    *,
    ...
    ``norual cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Norual dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `norual gateway status` without
