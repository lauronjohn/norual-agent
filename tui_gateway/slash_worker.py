"""Persistent slash-command worker — one NorualCLI per TUI session.

Protocol: reads JSON lines from stdin {id, command}, writes {id, ok, output|error} to stdout.
"""

# Stop a ``utils/`` (or ``proxy/``, ``ui/``) package in the launch directory
# from shadowing Norual's own top-level modules.  This worker is spawned as
...
def _prepare_slash_worker_runtime() -> None:
    """Start bounded MCP discovery before NorualCLI snapshots tools.

    Each slash_worker child is its own process — the parent ``norual serve``
...
def _run(cli: NorualCLI, command: str) -> str:
...
def main():
    ...
    # Start before the (hundreds-of-ms) NorualCLI build — that window is itself
    ...
    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = NorualCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
