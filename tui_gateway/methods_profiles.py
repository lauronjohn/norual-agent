...
def _(rid, params: dict) -> dict:
    """List Norual profiles (name, path, model, description, skill count).
...
def _(rid, params: dict) -> dict:
    ...
    ``norual setup`` in that flow to recover. A profile spawned as an
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        try:
            ...
            # Toolsets: the same filtered universe the `norual tools`
            ...
            for ts_name, ts_label, ts_desc in _get_effective_configurable_toolsets():
                ...
                # a per-profile editor — `norual tools` / Settings is where
