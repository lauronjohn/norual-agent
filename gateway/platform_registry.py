...
class PlatformEntry:
    ...
    # status displays (``norual setup``, ``norual status``, the dashboard
    ...
    # Env vars this platform needs (for ``norual setup`` display).
    ...
    # built-ins).  Used by ``norual gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # invocation -- including plain `norual chat`, which never touches any
        ...
        # `norual setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``norual setup``/``gateway status``, channel
