...
- POST /v1/chat/completions        — OpenAI Chat Completions format (stateless; opt-in session continuity via X-Norual-Session-Id header; opt-in long-term memory scoping via X-Norual-Session-Key header)
- POST /v1/responses               — OpenAI Responses API format (stateful via previous_response_id; X-Norual-Session-Key supported)
- GET  /v1/responses/{response_id} — Retrieve a stored response
- DELETE /v1/responses/{response_id} — Delete a stored response
- GET  /v1/models                  — lists hermes-agent and any configured model_routes aliases
- GET  /v1/capabilities            — machine-readable API capabilities for external UIs
- GET  /api/sessions               — list client-visible Norual sessions
- POST /api/sessions               — create an empty Norual session
...
def _hermes_version() -> str:
    """Return the canonical Norual Agent version string.
...
def _request_agent_overrides(
    ...
    *,
    ...
    ``direct_model_requests`` config value here, while Norual-native
    endpoints (session chat, /v1/runs) always allow it.  A request that
    sends an explicit ``provider`` is unambiguously Norual-aware and is
    always honored.
    """
...
def _derive_chat_session_id(
    ...
    the same Norual session (and therefore the same Docker container sandbox
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send an explicit ``provider`` — and the Norual-native session-chat
    ...
    def _resolve_request_profile(self, request: "web.Request"):
        ...
        if not getattr(cfg, "multiplex_profiles", False):
            ...
            # `norual peer dm mini/researcher` answered by the mini's default
    ...
    def _parse_session_key_header(
        ...
        """Extract and validate the ``X-Norual-Session-Key`` header.

        The session key is a stable per-channel identifier that scopes
        long-term memory (e.g. Honcho sessions) across transcripts.  It
        is independent of ``X-Norual-Session-Id``: callers may send
        ...
        """
        raw = request.headers.get("X-Norual-Session-Key", "").strip()
        if not raw:
            return None, None

        if not self._api_key:
            logger.warning(
                "X-Norual-Session-Key rejected: no API key configured. "
                "Set API_SERVER_KEY to enable long-term memory scoping."
            )
            return None, web.json_response(
                _openai_error(
                    "X-Norual-Session-Key requires API key authentication. "
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``norual sessions list``
    ...
    def _create_agent(
        ...
        by the client (via ``X-Norual-Session-Key``).  Unlike ``session_id``
        ...
        # (e.g. user ran `norual auth add openai-codex` without `norual model`),
    ...
    async def _handle_model_options(self, request: "web.Request") -> "web.Response":
        """GET /api/model/options — return Norual provider/model inventory.
        ...
        Norual provider catalog instead of scraping the single OpenAI-compatible
        `/v1/models` alias.
        """
        auth_err = self._check_auth(request)
        if auth_err:
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        every Norual version exposes the same endpoints.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({
            ...
            "runtime": {
                ...
                "split_runtime": False,
                "description": (
                    "The API server creates a server-side Norual AIAgent; "
            ...
            "features": {
                ...
                "session_continuity_header": "X-Norual-Session-Id",
                "session_key_header": "X-Norual-Session-Key",
    ...
    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        """GET /api/sessions — list persisted Norual sessions."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        # Exact-title lookup, used by `norual peer dm` to resolve a peer's
        ...
        if title_filter:
            ...
            if not sessions:
                ...
                # which would fail `norual peer dm` resolution and mint
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions -- create an empty Norual session row.
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        headers = {"X-Norual-Session-Id": effective_session_id or session_id}
        if gateway_session_key:
            headers["X-Norual-Session-Key"] = gateway_session_key
        runtime = {}
    ...
    async def _handle_session_chat_stream(self, request: "web.Request") -> "web.StreamResponse":
        ...
        headers = {
            ...
            "X-Norual-Session-Id": session_id,
        }
        if gateway_session_key:
            headers["X-Norual-Session-Key"] = gateway_session_key
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # stable per-channel identifier via X-Norual-Session-Key.  This
        # is independent of X-Norual-Session-Id: the key persists across
        ...
        # Allow caller to continue an existing session by passing X-Norual-Session-Id.
        ...
        provided_session_id = request.headers.get("X-Norual-Session-Id", "").strip()
        if provided_session_id:
            if not self._api_key:
                logger.warning(
                    "Session continuation via X-Norual-Session-Id rejected: "
            ...
            # conversation map to the same Norual session.  The first user
        ...
        response_headers = {
            "X-Norual-Session-Id": result.get("session_id", session_id),
        }
        if gateway_session_key:
            response_headers["X-Norual-Session-Key"] = gateway_session_key
        ...
        if not final_response and (is_failed or is_partial):
            ...
            response_headers["X-Norual-Completed"] = "false"
            response_headers["X-Norual-Partial"] = "true" if is_partial else "false"
            return web.json_response(err_body, status=502, headers=response_headers)

        # Soft-partial path: we have *some* text but the run did not complete
        # (e.g. truncation with partial buffered output). Still 200 but signal
        # truncation via finish_reason="length" + Norual-specific extras.
        response_data = {
            "id": completion_id,
        ...
        if is_partial or is_failed or not completed:
            ...
            response_headers["X-Norual-Partial"] = "true" if is_partial else "false"
            if err_msg:
                response_headers["X-Norual-Error"] = _redact_api_error_text(err_msg, limit=200)
    ...
    async def _write_sse_chat_completion(
        ...
        if session_id:
            sse_headers["X-Norual-Session-Id"] = session_id
        if gateway_session_key:
            sse_headers["X-Norual-Session-Key"] = gateway_session_key
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        # response and the X-Norual-Session-Id header.  Without this,
        ...
        response_headers = {"X-Norual-Session-Id": _effective_session_id}
        if gateway_session_key:
            response_headers["X-Norual-Session-Key"] = gateway_session_key
        return web.json_response(response_data, headers=response_headers)
            ...
            if role == "assistant" and msg.get("tool_calls"):
                for tc in msg["tool_calls"]:
                    ...
                    items.append({
                        ...
                        # Norual agent; they are replayed for structured tool
        ...
        def _run():
            ...
            with self._profile_scope(request_profile):
                ...
                try:
                    ...
                    # (e.g. X-Norual-Session-Id header) can track compression-
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
                    ...
                    with self._profile_scope(request_profile):
                        try:
                            ...
                            session_tokens = self._bind_api_server_session(
                                ...
                                # X-Norual-Session-Id equivalent) exactly like
        ...
        response_headers = (
            {"X-Norual-Session-Key": gateway_session_key} if gateway_session_key else {}
        )
        return web.json_response(
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # Store the adapter after native routes are registered. Local Norual-Relay
