...
# Reject epoch values before 2000-01-01T00:00:00Z: nothing in Norual' lifetime
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the Norual gateway lifecycle subcommand from a command line.
    ...
    ``--profile``/``-p`` selectors from anywhere in argv -- Norual's
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return "run"  # bare `norual gateway` defaults to `run`
        return filtered[i + 1]
    return None
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    use this broader matcher only when validating Norual-owned runtime records
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Norual gateway."""
...
def _get_code_identity_fields() -> dict[str, Any]:
    ...
    the identity into ``gateway_state.json`` lets `norual update` (and the
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `norual status` show per-profile
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # ``norual gateway stop`` on Windows would be misclassified as an
