"""Relay/connector support package for the Norual gateway.
...
def relay_connection_auth() -> tuple[Optional[str], Optional[str]]:
    ...
    Both come from enrollment (``norual gateway enroll`` writes them to
...
def relay_wake_url() -> Optional[str]:
    ...
    sets it explicitly (or passes ``--wake-url`` to ``norual gateway enroll``).
...
def relay_display_name() -> Optional[str]:
    ...
    if not value:
        ...
        # multi-agent scope it would prefix every reply "**Norual Agent:**" —
...
def _resolve_relay_identity_token() -> str:
    ...
    ``norual gateway enroll`` CLI. Three modes, in precedence order:
...
def self_provision_relay() -> bool:
    ...
    self-guarding:
      ...
      - A self-hosted operator who ran ``norual gateway enroll``: has a PINNED
