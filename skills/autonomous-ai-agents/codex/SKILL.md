...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Norual terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Norual itself, `model.provider: openai-codex` uses Norual-managed Codex
OAuth from `~/.hermes/auth.json` after `norual auth add openai-codex`. For the
...
## Norual Gateway Caveat

When invoking the Codex CLI from a Norual gateway/service context (for example,
