---
name: hermes-agent
description: "Use, configure, theme, extend, and orchestrate Norual Agent."
version: 3.2.0
...
Norual Agent is an open-source AI agent framework by Nous Research that runs in your terminal, a native desktop app, messaging platforms, and IDEs. It's in the same category as Claude Code (Anthropic), Codex (OpenAI), and OpenClaw — autonomous coding and task-execution agents that use tool calling to interact with your system. Norual works with any LLM provider (OpenRouter, Anthropic, OpenAI, Google, DeepSeek, xAI, local models, and 20+ others) and runs on Linux, macOS, Windows, and WSL.

What makes Norual different:

- **Self-improving through skills** — Norual learns from experience by saving reusable procedures as skills that load into future sessions.
...
- **Profiles** — run multiple independent Norual instances with isolated configs, sessions, skills, and memory.
...
This skill is a concise operating guide, not the complete source of truth for every Norual feature. If a Norual feature, command, or setting is not mentioned here or in a reference, do not treat that absence as evidence that it does not exist. Check the live repository and official docs before giving a negative answer.

Good verification targets, cheapest first:

- **Every shipped feature, one line each: https://hermes-agent.nousresearch.com/docs/llms.txt.** Start here for any "can Norual do X?" or "how do I do X?" — it indexes the entire documentation set with a link to the page that answers. It is generated from the docs tree on every build, so it is never behind the product. Fetch it with `web_extract`, or `curl -s https://hermes-agent.nousresearch.com/docs/llms.txt` when web tools are off. The whole documentation set in one file is at `/docs/llms-full.txt`.
- CLI commands: `norual --help`, `norual <command> --help`, and `hermes_cli/main.py`
- Source tree: https://github.com/NousResearch/hermes-agent

Never answer "Norual can't do that" from memory. Norual ships far more than this skill body describes, and the index exists so a negative answer is always checkable.
...
# Single query
norual chat -q "What is the capital of France?"
...
# Other surfaces
norual desktop                 # launch the native desktop app (alias: norual gui)
norual dashboard               # web admin panel + embedded chat
norual proxy                   # OpenAI-compatible local proxy backed by your OAuth provider
```
...
| User wants... | Load |
|---|---|
| **Anything not listed below — "can Norual do X?", "how do I set up X?"** | **https://hermes-agent.nousresearch.com/docs/llms.txt** |
...
| MCP servers (add, catalog, `norual mcp`) | `references/native-mcp.md` |
...
need more than their docs page. For everything else Norual ships, fetch
`llms.txt` and it maps the question to the page that answers it.

Two theming rules that hold even without loading the reference: **you apply skins yourself** (`norual config set display.skin <name>` — every surface repaints live within ~a second; don't tell the user to run `/skin`), and **to tweak one color, edit the ACTIVE skin** (`norual skin set <key> <hex>`) — never fork `default`, which drops the palette and resets the background.

## Spawning Additional Norual Instances

Run additional Norual processes as fully independent subprocesses — separate sessions, tools, and environments.
...
```
terminal(command="norual chat -q 'Research GRPO papers and write summary to ~/research/grpo.md'", timeout=300)

# Background for long tasks:
terminal(command="norual chat -q 'Set up CI/CD for ~/myapp'", background=true)
```

### Interactive PTY Mode (via tmux)

Norual uses prompt_toolkit, which requires a real terminal. Use tmux for interactive spawning:
...
```
# Agent A: backend
terminal(command="tmux new-session -d -s backend -x 120 -y 40 'norual -w'", timeout=10)
terminal(command="sleep 8 && tmux send-keys -t backend 'Build REST API for user management' Enter", timeout=15)

# Agent B: frontend
terminal(command="tmux new-session -d -s frontend -x 120 -y 40 'norual -w'", timeout=10)
...
terminal(command="tmux new-session -d -s resumed 'norual --continue'", timeout=10)

# Resume specific session
terminal(command="tmux new-session -d -s resumed 'norual --resume 20260225_143052_a1b2c3'", timeout=10)
```
...
- **Use `norual chat -q` for fire-and-forget** — no PTY needed
- **Use tmux for interactive sessions** — raw PTY mode has `\r` vs `\n` issues with prompt_toolkit
- **For scheduled tasks**, use the `cronjob` tool instead of spawning — handles delivery and retry
- **"delegate_task is capped at N" reports** — see `references/delegate-task-concurrency-diagnosis.md`. Three real cap paths in Norual; if none fired, the model is self-limiting and rationalising it as "the runtime caps."
...
- **Desktop app** (`norual desktop` / `norual gui`) — native Electron app for macOS/Linux/Windows: streaming chat, session list, Cmd+K palette, drag-and-drop files, native notifications, per-profile remote-gateway login. Extend it with UI plugins — `references/desktop-plugins.md`.
- **Web dashboard** (`norual dashboard`) — full admin panel: messaging channels, MCP catalog, webhooks, memory, profile builder, plus an embedded `norual --tui` chat. Secured behind an OAuth/token gate.
- **Ink TUI** (`norual --tui` or `display.interface: tui`) — terminal UI with docked widget apps — `references/tui-widgets.md`.
- **OpenAI-compatible proxy** (`norual proxy`) — a local OpenAI API backed by whichever OAuth provider you're signed into. Point Codex CLI, Aider, Cline, or any script at it — no API key.
...
- **Never hand-edit `config.yaml` for the user** — use `norual config set KEY VAL`; a stray indent can corrupt the file and break the live gateway.
