...
- A long-running process (norual gateway, tui_gateway) misbehaves and you can't restart it
...
The norual test runner and pytest both support this:
...
For long-lived processes: Norual gateway, tui_gateway, a daemon, a process that's already misbehaving and can't be restarted clean.
...
The easiest terminal-side DAP client is VS Code CLI or a small script. From inside Norual you have two practical options:
...
## Debugging Norual-specific Processes
...
### `tui_gateway` subprocess (spawned by `norual --tui`)
...
debugpy.wait_for_client()
```
Start `norual --tui`. The TUI will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.
...
9. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For Norual subagents, debug one process at a time.
