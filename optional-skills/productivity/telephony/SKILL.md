...
This optional skill gives Norual practical phone capabilities while keeping telephony out of the core tool list.
...
It does **not** turn Norual into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to Norual memory
...
### 1) "I want Norual to own a real phone number"
Use **Twilio**.
...
- pairs well with Norual `text_to_speech` plus a public file host or tunnel
...
```bash
norual skills search telephony
norual skills install official/productivity/telephony
```
...
Then save credentials into Norual:
...
```bash
python "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is Norual calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing Norual's existing `text_to_speech` support.

Use this when:
- you want the call to use Norual's configured TTS voice rather than Twilio `<Say>`
...
Recommended Norual TTS -> Twilio Play workflow:

1. Generate the audio with Norual `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask Norual to create the message audio with `text_to_speech`
...
Important note:
- Norual TTS is great for prerecorded outbound messages
...
- Norual STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to Norual memory.
...
- Do not store arbitrary third-party phone numbers in Norual memory.
