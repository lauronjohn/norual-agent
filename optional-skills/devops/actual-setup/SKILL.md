---
name: actual-setup
description: Set up Actual Computer (actual.inc) inference in Norual.
version: 2.0.0
author: shl0ms + Norual Agent
license: MIT
...
Sets up [actual.inc](https://actual.inc) (Actual Computer) as a Norual inference
...
- User has an `ac_` key and wants Norual routed through their Actual cluster.
...
- Norual has **first-class `actual` provider support** (provider id `actual`,
  aliases `actual-computer`, `actualcomputer`, `aci`). Do NOT configure Actual
  as a `custom_providers` / `providers.actual.*` entry on current Norual — the
...
3. Select provider + model:
   ```bash
   norual config set model.provider actual
   norual config set model.default "MODEL_ID_FROM_DISCOVERY"
   ```
4. Verify end-to-end:
   ```bash
   norual chat -Q -q "Reply with exactly: ACTUAL_OK" --provider actual -m MODEL_ID
   ```
...
2. Download and load a model (scriptable once authorized):
   ...
   ```
3. Point Norual at the daemon. `ACTUAL_BASE_URL` with a loopback host flips the
   ...
   ```bash
   norual config set model.provider actual
   norual config set model.default "INSTALLED_MODEL_NAME"
   ```
4. Verify (reduced toolset — see context-window pitfall below):
   ```bash
   norual chat -Q -q "Reply with exactly: LOCAL_OK" --provider actual -m INSTALLED_NAME -t file,web
   ```
...
1. **reasoning_effort trap (handled by Norual since the first-class provider).**
   ...
   request still 400s this way on an old Norual, set a per-model cap:
   `agent.reasoning_overrides.<model>: high` in config.yaml.
2. **Context-window overflow on small local models.** Norual' default toolset
   ...
   servers emit a bare `data: [DONE]` — Norual reports
...
5. **Do not create a custom provider named `actual`.** Older setup guides
   ...
   current Norual the built-in provider wins the name; stale custom blocks
...
```bash
# Relay:
norual chat -Q -q "Reply with exactly: ACTUAL_OK" --provider actual -m MODEL
# Local (small model — reduced toolset):
norual chat -Q -q "Reply with exactly: LOCAL_OK" --provider actual -m MODEL -t file,web
