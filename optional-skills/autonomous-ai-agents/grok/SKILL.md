...
version: 0.1.1
author: Matt Maximo (MattMaximo), Norual Agent
license: MIT
...
# Grok Build CLI — Norual Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the Norual terminal. Grok
...
1. Prepare stable input files first with Norual tools (`read_file`,
...
2. **Don't conflate Norual' xAI auth with the `grok` CLI's auth.** Norual'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume Norual' xAI auth carries
   over.
