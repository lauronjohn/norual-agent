---
name: honcho
description: Configure and troubleshoot Honcho memory for Norual.
version: 2.0.0
author: Norual Agent
license: MIT
...
# Honcho Memory for Norual

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Norual profile its own peer identity while sharing a unified view of the user.
...
```bash
norual honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Norual profile's AI peer
...
Honcho models conversations as interactions between **peers**. Norual creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Norual instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Norual run |
| `global` | Single session across all directories |

Manual override: `norual honcho map my-project-name`
...
Each Norual profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
```bash
norual profile create coder --clone
...
```bash
norual honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Norual when Honcho memory is active.
...
Run `norual honcho setup`. Ensure `memory.provider: honcho` is in `~/.hermes/config.yaml`.

### Memory not persisting across sessions
Check `norual honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `norual profile create <name> --clone`. For existing profiles: `norual honcho sync`.
...
| Command | Description |
|---------|-------------|
| `norual honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `norual honcho status` | Show resolved config, connection test, peer info for active profile |
| `norual honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `norual honcho disable` | Disable Honcho for the active profile |
| `norual honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `norual honcho peers` | Show peer identities across all profiles |
| `norual honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `norual honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `norual honcho sessions` | List known directory-to-session-name mappings |
| `norual honcho map <name>` | Map current working directory to a Honcho session name |
| `norual honcho identity` | Seed AI peer identity or show both peer representations |
| `norual honcho sync` | Create host blocks for all Norual profiles that don't have one yet |
| `norual honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Norual + Honcho |
| `norual memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `norual memory status` | Show active memory provider and config |
| `norual memory off` | Disable external memory provider |
