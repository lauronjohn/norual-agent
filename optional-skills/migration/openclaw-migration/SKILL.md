...
description: Import an OpenClaw setup (memories, skills) into Norual.
version: 1.0.0
author: Norual Agent (Nous Research)
license: MIT
platforms: [linux, macos, windows]
metadata:
  hermes:
    tags: [Migration, OpenClaw, Norual, Memory, Persona, Import]
    related_skills: [hermes-agent]
---

# OpenClaw -> Norual Migration

Use this skill when a user wants to move their OpenClaw setup into Norual Agent with minimal manual cleanup.
...
```bash
norual claw migrate              # Full interactive migration
norual claw migrate --dry-run    # Preview what would be migrated
norual claw migrate --preset user-data   # Migrate without secrets
norual claw migrate --overwrite  # Overwrite existing conflicts
norual claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `norual setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_hermes.py` to:

- import `SOUL.md` into the Norual home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Norual memory entries
- merge OpenClaw command approval patterns into Norual `command_allowlist`
- migrate Norual-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Norual working-directory configuration
- copy OpenClaw skills into `~/.hermes/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Norual workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.hermes/tts/`
- archive non-secret docs that do not have a direct Norual destination
...
With `--migrate-secrets`, it will also import a small allowlisted set of Norual-compatible secrets, currently:
...
Norual CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Norual. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Norual workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Norual skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Norual unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Norual targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Norual destination. Unsupported auth blobs must still be reported as skipped.
...
- Norual persona state imported
- Norual memory files populated with converted OpenClaw knowledge
