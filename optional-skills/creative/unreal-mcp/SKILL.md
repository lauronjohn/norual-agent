...
Companion skill for the `unreal-engine` entry in the Norual MCP catalog. The
...
Two halves, in this order: the editor side must be up before Norual connects.
...
### One-time, Norual side

    norual mcp install unreal-engine
...
Do NOT use `ModelContextProtocol.GenerateClientConfig` for Norual — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Norual connects
...
2. Start the Norual session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Norual
they appear as:
...
then happens at `norual mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Norual runs batched calls
...
- **Start order matters.** Editor + server up first, then the Norual
...
- **Timeouts on long operations.** Norual' per-call default is 120 s; asset
