...
short to a music video to an ASCII loop — in a Norual Kanban pipeline that
...
3. **Generates** a setup script that creates Norual profiles, project workspace, and the initial kanban task
...
For mapping role → which Norual skills + toolsets it loads, see
...
3. Creates each Norual profile via `norual profile create --clone`
...
7. Fires the initial `norual kanban create` task assigned to the director
...
```bash
norual kanban watch --tenant <project-tenant>     # live events
norual kanban list  --tenant <project-tenant>     # board snapshot
norual dashboard                                   # visual board UI
```
...
6. **The director never executes.** Even with the full `kanban + terminal +
   ...
   a `norual kanban create` call to a specialist profile. The kanban
