...
version: 1.0.0
author: Ben Barclay (benbarclay), Norual Agent
license: MIT
...
# MCP OAuth on a Remote Norual Gateway

## Overview

Norual' built-in MCP OAuth client runs a one-shot HTTP listener on `127.0.0.1:<port>`
inside the Norual process and registers that loopback address as the OAuth
`redirect_uri`. That works perfectly for a local CLI on the user's own machine.
It breaks completely when Norual runs as a remote gateway (container, VPS,
messaging bot), because the user's browser resolves `127.0.0.1` to the user's own
laptop, not the remote container — so the authorization code never reaches Norual.

This skill does the OAuth dance by hand and writes the resulting tokens into the
exact files Norual' token storage expects, so a subsequent `/reload-mcp` finds
...
2. Norual is running as a **remote gateway** (container, VPS, Docker, managed service) — NOT a local CLI on the user's laptop.
...
Do NOT use this for:
- **Local CLI Norual** — just set `auth: oauth` in `mcp_servers.<name>` and `/reload-mcp`. The built-in flow opens a browser and captures the callback on localhost. Works perfectly.
...
laptop and fails to connect. The callback never reaches the Norual process, the
flow times out, and `/reload-mcp` returns "No MCP tools available" with no detail.

Symptoms to recognize: `[xdg-open] <defunct>` processes under the norual user, an
...
already cover the deployment. When Norual detects a remote session it prints two
...
Both require an interactive terminal to the Norual host. The rest of this skill
is for when there is NO interactive TTY — Norual running purely as a messaging
gateway/bot where `/reload-mcp` triggers the flow with nobody at a prompt.

## Preferred Front Door: the Norual Dashboard (try this BEFORE manual token surgery)

A remote Norual gateway often also runs the **dashboard** web UI as a SEPARATE
process (e.g. `norual dashboard --host 0.0.0.0 --port <port>`; check with
`ps aux | grep 'norual dashboard'`). It exposes a connector/MCP console —
...
Norual' `NorualTokenStorage` would have written, so on `/reload-mcp` Norual finds
cached tokens and skips the browser flow entirely.
...
uses these same env vars, so if Norual' own auto-detect says "headless", the
...
```json
{
  "client_name": "Norual Agent (manual OAuth)",
...
### 8. Write tokens in Norual' exact schema

`tools/mcp_oauth.py::NorualTokenStorage` expects two files under
...
UA** — Cloudflare will 403 you even though Norual (which uses httpx) will succeed.
`scripts/diagnose-oauth-mcp.py` automates this smoke test.

### 11. Tell the user to run `/reload-mcp`

On reload, Norual sees `auth: oauth`, calls `NorualTokenStorage.get_tokens()`,
...
1. **Do not assume "headless" means "OAuth impossible."** The built-in flow works fine for local CLI; the issue is strictly remote deployments where the user's browser and the Norual process are on different machines. Check the execution environment before claiming OAuth isn't an option.
...
11. **Client info file is NOT optional.** Norual needs `<server>.client.json` to know the `client_id` for refresh grants. Skipping it means the first refresh fails and the user has to re-auth — writing both files is the whole point of this skill.
...
- **Don't use `mcp-remote` as a fallback.** It runs an npx subprocess whose OAuth callback server ALSO sits on the remote container's localhost — same problem. `mcp-remote` only helps when the MCP client doesn't speak remote HTTP at all (Norual does natively).
...
- **Don't claim Norual doesn't support a feature without reading the source.** Grep the source tree before making capability claims.
...
- `native-mcp` — general guide to configuring MCP in Norual. Authoritative config reference lives there.
- `mcporter` — the external CLI bridge, for ad-hoc MCP calls outside of Norual' config.
