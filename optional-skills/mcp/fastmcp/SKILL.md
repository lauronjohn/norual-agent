...
- smoke-test a server with the FastMCP CLI before wiring it into Norual or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Norual. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
When the goal is Norual integration, either:
...
### Norual cannot see the deployed server

The server-building part may be correct while the Norual config is not. Load the `native-mcp` skill and configure the server in `~/.hermes/config.yaml`, then restart Norual.
