...
row taxonomy is inferred from the atlas shape so Norual can render both legacy
...
# rows (1536x1872), while older Norual/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Norual' activity state names. They are not always identical to the
...
# Legacy Norual/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Norual activity names -> accepted row-name aliases in descending
