...
# un-throttled log fires on *every* selection. On Windows several Norual
...
# readiness handshake ("Timed out connecting to Norual backend after
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``norual model`` / ``norual auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Norual process
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        (e.g. setup wizard, ``norual auth status``) would land on
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # sync→POST→write-back sequence below must run atomically across Norual
    ...
    def _refresh_entry_impl(
        ...
        try:
            if self.provider == "anthropic":
                ...
                # refresh_token — single-use tokens consumed by another Norual
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI/nous — another Norual process
    ...
    def _available_entries(
        ...
        *pending_refresh* tuples so the caller can execute them outside the
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Norual profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `norual model` / `norual auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `norual model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``norual auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `norual auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `norual auth remove <provider> <N>` is stable across all source types.
    try:
    ...
    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Norual PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `norual setup` ("Claude
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `norual auth add nous --label <name>`).  Fall back to the
        ...
        try:
            ...
            # (norual auth remove copilot gh_cli) must not pay either on
        ...
        # Norual auth store or env vars, so resolve them here.
        ...
        # standard `norual auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `norual auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Norual auth store.  Without this gate the removal is instantly
        ...
        # Norual owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``norual model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``norual auth list`` reflects the logged-in state and so the pool
...
# authoritative source for Norual credentials. Stale env vars from parent
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Honour user suppression — `norual auth remove <provider> <N>` for an
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `norual auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons (device-code OAuth, claude_code) and Norual
