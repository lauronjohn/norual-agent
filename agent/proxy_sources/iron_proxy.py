...
* The ``iron-proxy`` binary is auto-installed into ``<hermes_home>/bin/iron-proxy``
  on first use.  Norual pins one upstream version (``_IRON_PROXY_VERSION``)
...
* A long-lived CA at ``<hermes_home>/proxy/ca.{crt,key}`` is generated on
  first ``norual egress setup``.  Sandboxes trust this CA so iron-proxy can
...
* The proxy runs as a managed subprocess (``norual egress start``), pidfile
...
# request needs the bearer key below.  ``norual egress reload`` is the
# client.
#
...
# credential (Norual' auth.py keys Google on both GEMINI_API_KEY and
...
def install_iron_proxy(*, force: bool = False) -> Path:
    ...
    these; the user-facing ``norual proxy install`` surface lets them
...
def iron_proxy_version(binary: Path) -> str:
    ...
    # Don't cache empty output — that would poison ``norual egress
...
def ensure_management_token(*, force: bool = False) -> str:
    ...
    ``norual egress reload`` reads the same file to authenticate.
    """
...
def reload_proxy() -> bool:
    ...
    if not pid or not _pid_alive(pid):
        raise RuntimeError(
            ...
            "Run `norual egress start`."
        )
    mgmt = _read_management_listen_from_config()
    if mgmt is None:
        raise RuntimeError(
            "The generated proxy.yaml has no management listener (written "
            "before reload support).  Re-run `norual egress setup` and use "
            "`norual egress restart` this one time."
        )
    token = _read_management_token()
    if not token:
        raise RuntimeError(
            "management.token is missing — re-run `norual egress setup`, "
            "then `norual egress restart`."
        )
...
def build_proxy_config(
    ...
    * ``secrets`` transform takes ``secrets:`` (plural), each with a
    ...
    return {
        ...
        # in ``ProxyStatus`` / ``norual egress status``.
        "metrics": {
            "listen": "127.0.0.1:0",
        },
        ...
        # pipeline — `norual egress reload` applies allowlist/token/mapping
...
def write_proxy_config(config: Dict) -> Path:
    ...
    try:
        import yaml  # PyYAML is already a Norual dep
...
def discover_uncovered_providers(
    *,
    ...
    The wizard and ``norual egress status`` use this to print a warning.
...
def merge_mappings(
    *,
    ...
    re-running ``norual egress setup`` should not invalidate the tokens
...
def start_proxy(
    *,
    ...
    if bin_path is None:
        raise RuntimeError(
            "iron-proxy binary not available — run `norual egress install`."
        )
    ...
    # close-on-success path, every restart leaked one fd in the Norual
    # process.
    #
    ...
    # ``norual egress stop`` can clean up the orphan.  Failure paths
    ...
    # hits Ctrl-C while waiting for ``norual egress start`` doesn't leak
    ...
    try:
        ...
        if existing_pid and _pid_alive(existing_pid):
            raise RuntimeError(
                ...
                f"Run `norual egress stop` if that proxy is stuck."
            )
            ...
            if access_token and project_id:
                ...
                if missing:
                    ...
                    if not (bitwarden_config or {}).get("allow_env_fallback"):
                        raise RuntimeError(
                            ...
                            f"`norual egress setup --no-bitwarden`, or set "
                ...
                if warnings:
                    logger.warning(
                        ...
                        "run `norual secrets bitwarden status` for detail.",
                        len(warnings),
                    )
            else:
