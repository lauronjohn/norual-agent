...
* No persisted JWT. ``azure-identity`` caches in-process and (where
  available) in the OS keychain or ``~/.IdentityService``. Norual does
...
class EntraIdentityConfig:
    ...
    Captures the Norual-managed Entra knobs we need outside Azure SDK
...
def _build_default_credential(config: EntraIdentityConfig) -> Any:
    ...
    Only Norual-selected knobs are passed as kwargs. Everything else
...
def build_credential(config: EntraIdentityConfig) -> Any:
    ...
    Norual processes use exactly one Entra config at a time (the
...
def has_azure_identity_credentials(scope: Optional[str] = None,
    ...
    error — never raises. Use for ``norual doctor`` /
    ``norual auth status`` / wizard preflight.
...
def describe_active_credential(config: Optional[EntraIdentityConfig] = None,
    ...
    Designed for ``norual doctor`` and the wizard preflight — never
    ...
    def _inject_bearer(request: "httpx.Request") -> None:
        try:
            ...
            logger.warning(
                ...
                "Run `norual doctor` or `az login` to recover.",
                exc,
            )
