"""Upload a Norual session transcript to Hugging Face as an agent trace.

Norual stores sessions in its own SQLite store (``hermes_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``norual trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Norual' secret redactor (``force=True``) unless the
...
# ---------------------------------------------------------------------------
# Conversion: Norual OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Norual' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Norual conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "3. Run /upload-trace again (or `norual trace upload`)."
)
