...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Norual starts and finds ``~/.openclaw/``.

    Points users at ``norual claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``norual claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Norual, run "
        "`norual claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `norual claw cleanup` (renames it to ~/.openclaw.pre-migration — "
