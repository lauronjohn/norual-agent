...
(``gateway/platforms/webhook.py``): inbound wakes Norual when the world
changes; outbound tells the world when Norual does something.
...
* Payloads are signed with HMAC-SHA256 (GitHub-style
  ``X-Norual-Signature-256: sha256=<hexdigest>`` over the raw body) when
    ...
    hooks:
      outbound:
        - url: https://ci.example.com/hermes-events
          ...
          name: ci-notify   # optional label for logs / `norual hooks list`
...
Headers::
    ...
    User-Agent:              Norual-Agent-Outbound-Webhook
    X-Norual-Event:          <hook event name>
    X-Norual-Delivery:       <delivery_id>
    X-Norual-Signature-256:  sha256=<hmac hexdigest>   # only when secret set
"""
...
def _serialize_payload(
    ...
    ``delivery_id`` is shared with the ``X-Norual-Delivery`` header so
...
def _build_delivery(
    ...
    if target.secret:
        ...
        headers["X-Norual-Signature-256"] = f"sha256={digest}"
    return {
        "url": target.url,
        "label": target.label,
        "event": event,
