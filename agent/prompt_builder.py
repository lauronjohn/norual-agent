...
DEFAULT_AGENT_IDENTITY = (
    "You are Norual Agent, the user's personal AI assistant (built on the "
    "Norual Agent platform by Nous Research). "
...
HERMES_AGENT_HELP_GUIDANCE = (
    "You run on Norual Agent (a personal fork of Norual Agent by Nous Research). "
    "When the user needs help with the agent itself — configuring, setting up, "
    "using, extending, or troubleshooting it — or when you need to understand "
    "your own features, tools, or capabilities, the documentation at "
    "https://hermes-agent.nousresearch.com/docs is your authoritative reference "
...
HERMES_AGENT_HELP_GUIDANCE_NO_SKILLS = (
    ...
    "and always holds the latest, most up-to-date information. Point the user "
    "there (or read it yourself if you have a way to fetch web content)."
)
...
KANBAN_GUIDANCE = (
    ...
    "every assignee in a real profile (`norual profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `norual kanban <verb>` for board operations. Use "
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "explicit Norual YOLO uses an unrestricted runtime after the user's "
        "launch/session risk acceptance. Permission mode and grants are fixed "
        "when Norual launches that runtime.\n\n"
        ...
        "run `norual computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Norual "
...
def hud_surface_note(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    HUD mode is a strip of Norual floating over another application, so the
    user is rarely asking about Norual — they are asking about the thing behind
    ...
    sentences = [
        "[Note: this message came from HUD mode — a small floating Norual "
        ...
        "rather than anything inside Norual. read_window_below identifies "
        "that app.",
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Norual terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "desktop": (
        "You are chatting inside the Norual desktop app — a graphical chat "
    ...
    "webui": (
        "You are in the Norual WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Norual itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Norual runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Norual itself is running. The host OS, home, and cwd "
                f"of the Norual process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Norual "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Norual
...
def _build_skills_system_prompt_inner(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Norual Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `hermes-agent` skill "
            "first. It has the actual commands (e.g. `norual config set …`, `norual tools`, "
            "`norual setup`) so you don't have to guess or invent workarounds.\n"
...
def build_context_files_prompt(
    ...
    # Never let a FALLBACK-picked directory inside the Norual install/source
    ...
    # explicitly configured cwd is honored verbatim — the Norual tree is a
    ...
    # their launch dir IS the user's shell cwd (developing Norual in-tree).
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the Norual install tree (%s) — set terminal.cwd to "
