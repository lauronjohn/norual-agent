...
def _media_caption_split(text, media_files, *, max_caption_len):
    ...
    across every standalone sender. ``norual send`` (and the send_message
    ...
    ``norual send --to whatsapp "MEDIA:/x.png This Caption"`` into two parts.
    ...
    if not chat_id:
        ...
        if home:
            ...
            return tool_error(
                ...
                f"or set a home channel via: norual config set {home_env} <channel_id>"
            )
...
async def _send_matrix_via_adapter(pconfig, chat_id, message, media_files=None, thread_id=None):
    ...
    is running (standalone cron, ``norual send`` CLI).
    """
    media_files = media_files or []
...
#   - the ``norual send`` CLI command (hermes_cli/send_cmd.py)
