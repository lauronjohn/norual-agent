...
`norual skills publish` for sharing. No new source type, no new store, no new
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    norual = meta.get("hermes") if isinstance(meta, dict) else None
    blueprint = norual.get("blueprint") if isinstance(norual, dict) else None
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    to ``norual skills publish`` to share. ``body`` is the plain-language
