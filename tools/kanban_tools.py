...
orchestrator work. A normal ``norual chat`` session still sees **zero**
kanban tools in its schema unless configured.

Why tools instead of just shelling out to ``norual kanban``?

1. **Backend portability.** A worker whose terminal tool points at Docker
   / Modal / Singularity / SSH would run ``norual kanban complete …``
...
Humans continue to use the CLI (``norual kanban …``), the dashboard
(``norual dashboard``), and the slash command (``/kanban …``) — all
...
def _check_kanban_mode() -> bool:
    ...
    Humans running ``norual chat`` without the kanban toolset see zero
...
def _connect(board: Optional[str] = None):
    ...
    the env-pinned active board without restarting Norual.
    """
...
def _handle_attach_url(args: dict, **kw) -> str:
    ...
    The agent passes a URL; Norual downloads it (with the shared size cap)
...
KANBAN_ATTACH_URL_SCHEMA = {
    ...
    "description": (
        "Attach a file to a task by URL — Norual downloads it server-side "
