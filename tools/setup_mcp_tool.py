...
``norual mcp install <name>`` in the terminal.
"""
...
def setup_mcp_tool(
    ...
    if callback is None:
        return tool_error(
            "setup_mcp is only available in the Norual desktop app. Use the "
            "terminal instead: `norual mcp install <name>` for catalog entries, "
            "`norual mcp login <name>` for OAuth."
        )
...
SETUP_MCP_SCHEMA = {
    ...
    "description": (
        ...
        "Norual desktop chat. The card lets them install a catalog entry, "
        ...
        "continue without the server. Catalog names: run `norual mcp catalog` "
