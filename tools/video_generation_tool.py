...
provider** — turn on a backend by enabling a plugin (``norual plugins
enable video_gen/<name>``) and selecting it in ``norual tools`` → Video
Generation.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `norual tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"installed video gen backends, or `norual tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `norual tools` → "
...
# `norual tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`norual tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if provider is None:
        parts.append(
            ...
            "until the user picks one via `norual tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
