...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (Norual convention — don't recursively snapshot siblings)
    ".worktrees/",
...
def _load_ledger(store: Path, dir_hash: str) -> Dict[str, Dict]:
    ...
    ``write_file`` / ``patch`` produced, so restores can tell "Norual wrote
...
def _git_env(
    ...
    The shared store is internal Norual infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``norual checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `norual checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    _run_git(["config", "user.name", "Norual Checkpoint"], store, cfg_wd)
...
class CheckpointManager:
    ...
    def record_agent_write(self, file_path: str) -> None:
        """Record the content hash of a file Norual just successfully wrote.
        ...
        recorded hash was hand-edited by the user after Norual last touched
    ...
    def safe_restore_plan(self, working_dir: str, commit_hash: str) -> Dict:
        ...
        still matches what Norual last wrote (per the agent-write ledger)
        and ``skipped`` lists files the user hand-edited after Norual'
        last write or that Norual never wrote at all.
        """
        ...
        ledger = _load_ledger(store, dir_hash)
        if not ledger:
            # No agent-write ledger yet (pre-existing store, or Norual has
        ...
        for rel in names_out.splitlines():
            ...
            if recorded is None:
                # Norual never wrote this file (or the ledger predates it) —
                # do not touch it in safe mode.
                skipped.append(rel)
                continue
            current = _hash_file(abs_path)
            if current is None:
                # File deleted since Norual wrote it: restoring it back is
                # safe — its last content was Norual-authored.
                restore.append(rel)
    ...
    def session_diff(self, working_dir: str) -> Dict:
        ...
        This powers ``/diff session``.  It answers "what has Norual changed
        ...
        *retained* checkpoint may predate the current session (or, after
        ...
        Norual changed", not an exact per-session ledger.
    ...
    def restore(
        ...
        hand-edited after Norual' last write — per the agent-write ledger —
        are left untouched, and only Norual-authored changes are reverted.
        ...
        if restore_paths is not None:
            ...
            # Norual-created files absent from it (delete to restore state).
...
# ---------------------------------------------------------------------------
# Public helpers for `norual checkpoints` CLI
# ---------------------------------------------------------------------------
