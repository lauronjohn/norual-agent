...
The recovery path lives in ``NorualCLI._recover_terminal_after_interrupt()``,
...
@pytest.fixture
def bare_cli():
    """A NorualCLI with no __init__ — we only exercise the recovery helper."""
    return object.__new__(NorualCLI)


class TestRecoverTerminalAfterInterrupt:
    """Directly exercise NorualCLI._recover_terminal_after_interrupt()."""
...
class TestFinallyBlockWiring:
    ...
    def test_recovery_is_invoked_behind_interrupt_guard(self):
        src = inspect.getsource(NorualCLI.run)
    ...
    def test_recovery_helper_exists(self):
        assert hasattr(NorualCLI, "_recover_terminal_after_interrupt")
        assert callable(NorualCLI._recover_terminal_after_interrupt)
