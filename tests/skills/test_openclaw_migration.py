...
"""
    ...
    assert "Tyler Williams > Active Projects: Norual Agent" in entries
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``Norual``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "Norual prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told Norual to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "Norual config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "Norual uses tools well"
    # All-lowercase matches → lowercase ``hermes``; this preserves the
    # real filesystem path ``~/.hermes`` (Norual home) when rebranding
    # memory entries that reference ``~/.openclaw`` or ``openclaw`` prose.
    assert mod.rebrand_text("openclaw should always respond concisely") == "norual should always respond concisely"
