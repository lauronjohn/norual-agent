"""Tests for ``norual dashboard --stop`` / ``--status`` flags.

These flags share the detection + kill path with the post-``norual update``
...
class TestDashboardStatus:
    def test_status_no_processes(self, capsys):
        ...
        assert "No norual dashboard processes running" in out

    def test_status_with_processes(self, capsys):
        processes = [
            (12345, "norual dashboard --port 9119"),
        ...
        assert exc.value.code == 0
        out = capsys.readouterr().out
        assert "2 norual dashboard process(es) running" in out
...
class TestDashboardStop:
    ...
    def test_stop_kills_and_exits_zero_when_all_killed(self, capsys):
        ...
        # for the post-`norual update` path).
...
class TestLifecycleFlagsTakePrecedence:
    ...
    who typed ``norual dashboard --stop`` must not end up ALSO starting
    a new server."""
...
class TestArgparseWiring:
    ...
    ``norual dashboard --stop`` / ``--status`` actually parse."""
