...
from hermes_cli.console_engine import NorualConsoleEngine, run_console_repl
...
def test_sessions_export_rejects_oversized_single_before_touching_output(
    ...
    result = NorualConsoleEngine().execute(
...
def test_repl_runs_non_interactive_lines_without_prompts(_isolate_hermes_home):
    ...
    assert code == 0
    assert "Norual Console" in stdout.getvalue()
...
def test_console_checkpoints_prune_does_not_reprompt_for_orphans(
    ...
    result = NorualConsoleEngine().execute("checkpoints prune", confirmed=True)
