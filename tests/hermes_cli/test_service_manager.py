...
# unprivileged norual user from every lifecycle op. These tests run
...
def test_render_run_script_uses_replace_to_take_over_stale_holder() -> None:
    ...
    ``norual gateway run``, an agent action, the Open WebUI helper) holds
    ...
    # Root profile: bare `norual gateway run --replace` (no -p flag).
    assert "norual gateway run --replace" in default_text
    assert "norual -p default" not in default_text
...
#
# `norual gateway stop` inside a container dispatches through
...
def test_s6_log_run_creates_leaf_as_hermes_without_chown(
    ...
    assert 's6-setuidgid norual mkdir -p "$log_dir"' in log_text
    assert 's6-setuidgid norual rm -f "$log_dir/lock"' in log_text
    assert 'else\n  mkdir -p "$log_dir"\n  rm -f "$log_dir/lock"\nfi\n' in log_text
    ...
    mkdir_as_hermes_idx = log_text.index('s6-setuidgid norual mkdir -p "$log_dir"')
    rm_as_hermes_idx = log_text.index('s6-setuidgid norual rm -f "$log_dir/lock"')
...
def test_s6_log_run_never_invokes_chown_with_symlinked_log_dir(tmp_path) -> None:
    ...
    # a real norual uid cannot delete a foreign root-owned lock.
    ...
    (bin_dir / "rm").write_text(
        "#!/bin/sh\n"
        # Privilege-dropped: no-op. Models that norual cannot unlink a foreign
