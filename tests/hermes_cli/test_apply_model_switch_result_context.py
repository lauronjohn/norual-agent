...
``NorualCLI._apply_model_switch_result()`` printed ``ModelInfo.context_window``
...
def _run_display(monkeypatch, result):
    ...
    cli_mod.NorualCLI._apply_model_switch_result(_StubCLI(), result, False)
    return captured
...
def test_global_switch_clears_context_pin_owned_by_previous_route(monkeypatch):
    ...
    with (
        ...
        cli_mod.NorualCLI._apply_model_switch_result(cli, result, True)
