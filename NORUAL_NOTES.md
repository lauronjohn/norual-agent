...
| `hermes_cli/provider_cmd.py` + `cli.py` | `norual provider --remove <id>` / `/provider --remove <id>`: deletes the provider's env keys (and `providers.<id>` config entry when present) via `remove_env_value`/`unset_config_value`; provider flips back to "(no key)" and leaves the configured set | Fork addition |
| `agent/turn_context.py` | Session auto-titling strips the one-shot `[Note: model was just switched ...]` prefix the CLI prepends after /model — otherwise the INSTANT derived title became the note, showing as a truncated badge in the status bar's right edge | Keep the strip if upstream changes the note format |
| `cli.py` + `hermes_cli/banner.py` | UI: streaming response/reasoning boxes now word-wrap to the box's inner width (`_wrap_stream_line`, ANSI-aware, hard-slices over-long tokens) so text stays inside the border and nothing gets cut off; banner logo centered | Fork behavior change; upstream relied on terminal soft-wrap |
| `cli.py` | Composer placeholder is now a non-insertable COMMAND suggestion hint (`_command_suggestion_hint()`: `/help · /model · /models · /provider · /skin`) shown on idle+empty input; Tab on empty input opens the command completion menu. The context-based placeholder was dropped ("never mind"), and the rotating `COMPOSER_PLACEHOLDERS` hints remain unused | Fork behavior change; upstream = rotating canned hints |
| `cli.py` | LIVE spinner: a daemon ticker (~7fps, `_spinner_anim_loop`) drives a PULSING THUNDERBOLT (`⚡︎` in the skin's `ui_accent` color, brightness ramps 40%→100%→40%). The bolt is a prompt_toolkit STYLED FRAGMENT with a per-frame hex color (`_bolt_hex_color`) — raw ANSI in a plain string renders as literal `38;2;…` text, which was the bug; fragment styling emits real escapes the terminal interprets. Static spinner text; "thinking" placeholder covers the pre-first-token wait | Fork behavior change; upstream = static spinner text |
