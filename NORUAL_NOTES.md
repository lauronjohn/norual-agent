...
| `cli.py` | LIVE spinner: a daemon ticker (~7fps, `_spinner_anim_loop`) drives a simple BREATHING PULSE bar (`▁▂▃▄▅▆▇█▇▆▅▄▃▂`) before the (static) spinner text; "thinking" placeholder covers the pre-first-token wait. Rotation was dropped per user request — keep it simple | Fork behavior change; upstream = static spinner text |
