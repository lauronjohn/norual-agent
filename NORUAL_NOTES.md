...
| `cli.py` | Composer placeholder is now CONTEXT-AWARE (`_context_composer_placeholder()`): shows the last user message (trimmed, 60 chars) after there's history; empty input on a fresh session shows nothing. Replaces the rotating `COMPOSER_PLACEHOLDERS` hint; Tab inserts the same context text. `tips.py` placeholder data left in place (unused) | Fork behavior change; upstream = rotating canned hints |
