...
| `cli.py` | LIVE spinner: a daemon ticker (~7fps, `_spinner_anim_loop`) drives a PULSING THUNDERBOLT (`⚡︎` in the skin's `ui_accent` color, brightness ramps 40%→100%→40% via ANSI truecolor; text-presentation selector keeps it monochrome/colored rather than emoji-yellow). Static spinner text; "thinking" placeholder covers the pre-first-token wait | Fork behavior change; upstream = static spinner text |
