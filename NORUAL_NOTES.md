...
| `cli.py` | LIVE spinner animation: a daemon ticker (~7fps, `_spinner_anim_loop` mirroring the pet loop) rotates the skin's thinking/waiting faces (`(0x) (>>) (◉) …`) and thinking verbs ("cracking shells", "rooting the filesystem"…) while the agent/command runs; a default animated verb covers the pre-first-token wait. Upstream spinner was static text | Fork behavior change; upstream = static spinner text |
